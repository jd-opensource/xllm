...
MUSAQwen3DecoderImpl::MUSAQwen3DecoderImpl(const ModelContext& context) {
  ...
  layers_.push_back(register_module(
      "musa_attn",
      MusaAttention(model_args, quant_args, parallel_args, options)));
...
void MUSAQwen3DecoderImpl::load_state_dict(const StateDict& state_dict) {
  for (auto&& mod : layers_) {
    mod->load_state_dict(state_dict);
  }
}

torch::Tensor MUSAQwen3DecoderImpl::forward(
    torch::Tensor& x,
