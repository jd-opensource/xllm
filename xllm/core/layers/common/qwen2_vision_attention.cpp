...
Qwen2VisionAttentionImpl::Qwen2VisionAttentionImpl(const ModelContext& context,
  ...
  const int64_t hidden_size = model_args->mm_hidden_size();
  const int64_t num_heads = model_args->mm_num_attention_heads();
  const int64_t tp_size = parallel_args.tp_group_->world_size();
  CHECK(num_heads % tp_size == 0);

  tp_group_ = parallel_args.tp_group_;
  hidden_size_per_attention_head_ = model_args->mm_head_dim();
