...
AttentionMetadata build_attention_metadata(
  ...
  const auto& meta = forward_input.meta;
  const auto& attention = forward_input.attention;
  const auto& graph = forward_input.graph;
  const auto* llmrec_params = forward_input.llmrec_params();
  const bool enable_cuda_graph =
      forward_input.enable_cuda_graph || forward_input.enable_graph;
  const bool is_spec_verify = forward_input.is_spec_verify;
  const auto& input_embedding = forward_input.embedding.input_embedding;
...
#if defined(USE_NPU)
  attn_metadata.is_spec_verify = is_spec_verify;
  ...
  if (attn_metadata.use_expanded_decode_for_spec_verify_attention) {
    attn_metadata.expanded_kv_seq_lens = graph.expanded_kv_seq_lens;
    attn_metadata.expanded_block_table = graph.expanded_block_tables;
    attn_metadata.expanded_paged_attention_tiling_data =
        graph.expanded_tiling_data;
    if (!graph.expanded_kv_seq_lens_vec.empty()) {
      attn_metadata.expanded_kv_seq_lens_host =
          torch::tensor(graph.expanded_kv_seq_lens_vec, torch::kInt);
    }
  }
  ...
  if (!attention.host.q_seq_lens.empty()) {
    attn_metadata.q_seq_lens_host =
        torch::tensor(attention.host.q_seq_lens, torch::kInt);
  }
  if (!attention.host.kv_seq_lens.empty()) {
    attn_metadata.kv_seq_lens_host =
        torch::tensor(attention.host.kv_seq_lens, torch::kInt);
  }
  if (!attention.host.q_cu_seq_lens.empty()) {
    attn_metadata.q_cu_seq_lens_host_vec.reserve(
        attention.host.q_cu_seq_lens.size());
    for (int32_t len : attention.host.q_cu_seq_lens) {
  ...
  if (!attention.host.kv_seq_lens.empty()) {
    ...
    kv_cu.reserve(attention.host.kv_seq_lens.size());
    int64_t total = 0;
    for (int32_t len : attention.host.kv_seq_lens) {
      total += len;
  ...
  if (!is_decode) {
    ...
    if (attention.device.q_seq_lens.defined()) {
      mask_device = attention.device.q_seq_lens.device();
    } else if (input_embedding.defined()) {
      mask_device = input_embedding.device();
    }
  ...
  if (attention.device.q_seq_lens.defined()) {
    ...
    torch::Tensor q_cu_seq_lens = attention.device.q_cu_seq_lens;
    ...
    const bool q_cu_has_leading_zero =
        !attention.host.q_cu_seq_lens.empty() &&
        attention.host.q_cu_seq_lens.front() == 0;
    if (graph.tiling_data.defined() || q_cu_has_leading_zero) {
...
AttentionMetadata AttentionMetadataBuilder::build(
  ...
  return AttentionMetadataBuilder::build(
      forward_input, enable_mla, "float", attn_mask, device);
}
...
AttentionMetadata AttentionMetadataBuilder::build(
  ...
  return build_attention_metadata(
      forward_input, enable_mla, compute_dtype, device, attn_mask);
}
