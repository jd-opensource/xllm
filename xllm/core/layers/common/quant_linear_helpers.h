...
// ── DCU W8A8 Forward (Dynamic) ──────────────────────────────────────────────

#if defined(USE_DCU)
inline torch::Tensor dcu_w8a8_dynamic_linear_forward(
    const torch::Tensor& input,
    const torch::Tensor& weight,
    const torch::Tensor& weight_scale,
    const std::optional<torch::Tensor>& bias,
    at::ScalarType output_dtype) {
  kernel::ScaledQuantizeParams quantize_params;
  ...
  std::tie(quantized_input, input_scale) =
      kernel::scaled_quantize(quantize_params);

  kernel::ScaledMatmulParams matmul_params;
  ...
  return kernel::scaled_matmul(matmul_params);
}
#endif  // USE_DCU
