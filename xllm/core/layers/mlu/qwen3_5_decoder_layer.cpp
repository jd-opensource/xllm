...
torch::Tensor Qwen3_5DecoderLayerImpl::run_moe(torch::Tensor x,
                                               const ForwardInput& input) {
  const bool enable_moe_all2all =
      use_moe_all2all(enable_deep_ep_, input.parallel);
  if (need_dp_moe_gather(parallel_args_, enable_moe_all2all)) {
    x = gather_dp_tokens(x, input.parallel, parallel_args_);
    x = moe_mlp_->forward_experts(x, enable_moe_all2all);
    return get_dp_local_slice(x, input.parallel, parallel_args_);
  }
