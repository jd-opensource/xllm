...
#include "glm4_moe_decoder_manual_loader.h"
...
Glm4MoeDecoderManualLoader::Glm4MoeDecoderManualLoader(
...
void Glm4MoeDecoderManualLoader::resize_experts_weights(
...
void Glm4MoeDecoderManualLoader::load_state_dict(const StateDict& state_dict) {
...
void Glm4MoeDecoderManualLoader::verify_loaded_weights() const {
  for (const auto& [name, index] : weight_mapping_) {
    ...
    CHECK(at_host_weight_tensors_[index].defined() &&
          at_host_weight_tensors_[index].numel() > 0)
        << layer_id_ << "-weight is not loaded for " << name;
  }
}

void Glm4MoeDecoderManualLoader::merge_host_at_weights() {
  merge_shared_experts_weights();
  if (layer_id_ >= prefill_param_firstKDenseReplace_) {
    merge_experts_weights();
  }
  at_host_weight_tensors_[IN_QKV_WEIGHT_0] =
      torch::cat({at_host_weight_tensors_[IN_QKV_WEIGHT_0],
                  at_host_weight_tensors_[IN_QKV_WEIGHT_1],
                  at_host_weight_tensors_[IN_QKV_WEIGHT_2]},
                 0)
          .contiguous();
  at_host_weight_tensors_[IN_QKV_WEIGHT_1] = torch::zeros(
      {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));
  at_host_weight_tensors_[IN_QKV_WEIGHT_2] = torch::zeros(
      {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));

  at_host_weight_tensors_[IN_QKV_BIAS_0] =
      at_host_weight_tensors_[IN_QKV_BIAS_0].squeeze();
  at_host_weight_tensors_[IN_QKV_BIAS_1] =
      at_host_weight_tensors_[IN_QKV_BIAS_1].squeeze();
  at_host_weight_tensors_[IN_QKV_BIAS_2] =
      at_host_weight_tensors_[IN_QKV_BIAS_2].squeeze();

  at_host_weight_tensors_[IN_QKV_BIAS_0] =
      torch::cat({at_host_weight_tensors_[IN_QKV_BIAS_0],
                  at_host_weight_tensors_[IN_QKV_BIAS_1],
                  at_host_weight_tensors_[IN_QKV_BIAS_2]},
                 0)
          .contiguous();
  at_host_weight_tensors_[IN_QKV_BIAS_1] = torch::zeros(
      {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));
  at_host_weight_tensors_[IN_QKV_BIAS_2] = torch::zeros(
      {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));

  if (quantize_type_.compare("w8a8_dynamic") == 0) {
    at_host_weight_tensors_[IN_QKV_DESCALE_0] =
        at_host_weight_tensors_[IN_QKV_DESCALE_0].squeeze();
    at_host_weight_tensors_[IN_QKV_DESCALE_1] =
        at_host_weight_tensors_[IN_QKV_DESCALE_1].squeeze();
    at_host_weight_tensors_[IN_QKV_DESCALE_2] =
        at_host_weight_tensors_[IN_QKV_DESCALE_2].squeeze();

    at_host_weight_tensors_[IN_QKV_DESCALE_0] =
        torch::cat({at_host_weight_tensors_[IN_QKV_DESCALE_0],
                    at_host_weight_tensors_[IN_QKV_DESCALE_1],
                    at_host_weight_tensors_[IN_QKV_DESCALE_2]},
                   0)
            .contiguous();

    at_host_weight_tensors_[IN_QKV_DESCALE_1] = torch::zeros(
        {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));
    at_host_weight_tensors_[IN_QKV_DESCALE_2] = torch::zeros(
        {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));

    at_host_weight_tensors_[IN_QKV_DENSE_BIAS] = torch::zeros(
        {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));
    at_host_weight_tensors_[IN_QKV_DENSE_DESCALE] = torch::zeros(
        {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));

    at_host_weight_tensors_[IN_QKV_OFFSET_0] =
        at_host_weight_tensors_[IN_QKV_OFFSET_0].to(torch::kInt8);
    at_host_weight_tensors_[IN_QKV_OFFSET_1] = torch::zeros(
        {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));
    at_host_weight_tensors_[IN_QKV_OFFSET_2] = torch::zeros(
        {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));
    at_host_weight_tensors_[IN_QKV_DENSE_OFFSET] =
        at_host_weight_tensors_[IN_QKV_DENSE_OFFSET].contiguous().view(-1);

    at_host_weight_tensors_[IN_QKV_SCALE_1] = torch::zeros(
        {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));
    at_host_weight_tensors_[IN_QKV_SCALE_2] = torch::zeros(
        {1}, torch::TensorOptions().dtype(torch::kFloat16).device(torch::kCPU));
    at_host_weight_tensors_[IN_QKV_DENSE_SCALE] =
        at_host_weight_tensors_[IN_QKV_DENSE_SCALE].contiguous().view(-1);
  }
}

void Glm4MoeDecoderManualLoader::process_expert_weights(
...
void Glm4MoeDecoderManualLoader::process_shared_expert_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
...
void Glm4MoeDecoderManualLoader::process_mlp_common_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
  const auto& weight_mapping = (quantize_type_.compare("w8a8_dynamic") == 0)
...
void Glm4MoeDecoderManualLoader::process_general_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
  const auto& weight_mapping = (quantize_type_.compare("w8a8_dynamic") == 0)
  ...
  if (quantize_type_.compare("w8a8_dynamic") == 0) {
    ...
    if (it != SPECIAL_MULTI_ASSIGN_W8A8.end()) {
      for (int idx : it->second) {
        at_host_weight_tensors_[idx] = tmp_tensor;
      }
      return;
    }
  }
...
torch::Tensor Glm4MoeDecoderManualLoader::get_sharded_tensor(
    const StateDict& state_dict,
    const std::string& name,
    int dim) {
  if (parallel_args_.world_size() > 1) {
    ...
    return state_dict.get_tensor(name).to(torch::kCPU);
  }
}
...
std::string Glm4MoeDecoderManualLoader::extract_endswith(
...
int Glm4MoeDecoderManualLoader::extract_expert_index(const std::string& name) {
...
void Glm4MoeDecoderManualLoader::merge_shared_experts_weights() {
  ...
  if (layer_id_ >= prefill_param_firstKDenseReplace_) {
    ...
    if (quantize_type_ == "w8a8_dynamic") {
      ...
      at_host_weight_tensors_[IN_MLP_GATEUP_OFFSET_SHARED_EXPERT] =
          at_host_weight_tensors_[IN_MLP_GATEUP_OFFSET_SHARED_EXPERT].squeeze();
      at_host_weight_tensors_[IN_MLP_GATEUP_SCALE_SHARED_EXPERT] =
          at_host_weight_tensors_[IN_MLP_GATEUP_SCALE_SHARED_EXPERT].squeeze();
      at_host_weight_tensors_[IN_MLP_DOWN_OFFSET_SHARED_EXPERT] =
          at_host_weight_tensors_[IN_MLP_DOWN_OFFSET_SHARED_EXPERT].squeeze();
      at_host_weight_tensors_[IN_MLP_DOWN_SCALE_SHARED_EXPERT] =
          at_host_weight_tensors_[IN_MLP_DOWN_SCALE_SHARED_EXPERT].squeeze();
    }
  } else {
...
void Glm4MoeDecoderManualLoader::merge_experts_weights() {
  try {
    ...
    if (quantize_type_.compare("w8a8_dynamic") == 0) {
      ...
      at_host_weight_tensors_[IN_MLP_GATEUP_OFFSET] =
          merge_experts_weights(experts_weights_["gate_proj.weight_offset"],
                                experts_weights_["up_proj.weight_offset"]);
      at_host_weight_tensors_[IN_MLP_GATEUP_SCALE] =
          merge_experts_weights(experts_weights_["gate_proj.weight_scale"],
                                experts_weights_["up_proj.weight_scale"]);
      at_host_weight_tensors_[IN_MLP_GATEUP_WEIGHT] = mlp_gateup_weight;
    } else {
      mlp_gateup_weight =
          merge_experts_weights(experts_weights_["gate_proj.weight"],
                                experts_weights_["up_proj.weight"],
                                /*transpose=*/false);
      at_host_weight_tensors_[IN_MLP_GATEUP_WEIGHT] =
  ...
  try {
    ...
    at_host_weight_tensors_[IN_MLP_DOWN_WEIGHT] = mlp_down_weight.contiguous();

    if (quantize_type_.compare("w8a8_dynamic") == 0) {
      at_host_weight_tensors_[IN_MLP_DOWN_OFFSET] =
          merge_experts_weights(experts_weights_["down_proj.weight_offset"]);
      at_host_weight_tensors_[IN_MLP_DOWN_SCALE] =
...
torch::Tensor Glm4MoeDecoderManualLoader::merge_experts_weights(
...
bool Glm4MoeDecoderManualLoader::is_nz_format_tensor(int weight_index) {
  return weight_index == IN_MLP_GATEUP_WEIGHT ||
         weight_index == IN_MLP_DOWN_WEIGHT;
}

int Glm4MoeDecoderManualLoader::get_mapped_index(
    const std::string& name,
