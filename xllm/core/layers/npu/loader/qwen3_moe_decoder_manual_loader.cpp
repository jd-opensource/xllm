...
#include "qwen3_moe_decoder_manual_loader.h"
...
Qwen3MoeDecoderManualLoader::Qwen3MoeDecoderManualLoader(
...
void Qwen3MoeDecoderManualLoader::load_state_dict(const StateDict& state_dict) {
...
void Qwen3MoeDecoderManualLoader::verify_loaded_weights() const {
  verify_loaded_weights("qwen3_moe");
}

void Qwen3MoeDecoderManualLoader::verify_loaded_weights(
...
void Qwen3MoeDecoderManualLoader::merge_experts_weights() {
...
torch::Tensor Qwen3MoeDecoderManualLoader::merge_experts_weights(
...
std::string Qwen3MoeDecoderManualLoader::extract_endswith(
...
int Qwen3MoeDecoderManualLoader::extract_expert_index(const std::string& name) {
...
void Qwen3MoeDecoderManualLoader::resize_experts_weights(
...
void Qwen3MoeDecoderManualLoader::process_expert_weights(
...
int Qwen3MoeDecoderManualLoader::get_mapped_index(
    const std::string& name,
...
void Qwen3MoeDecoderManualLoader::process_mlp_common_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
  const auto& weight_mapping = (quantize_type_.compare("w8a8_dynamic") == 0)
...
void Qwen3MoeDecoderManualLoader::process_general_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
  const auto& weight_mapping = (quantize_type_.compare("w8a8_dynamic") == 0)
...
torch::Tensor Qwen3MoeDecoderManualLoader::get_sharded_tensor(
    const StateDict& state_dict,
    const std::string& name,
    int dim) {
...
void Qwen3MoeDecoderManualLoader::merge_host_at_weights() {
  ...
  at_host_weight_tensors_[IN_QKV_WEIGHT_0] =
      torch::cat({at_host_weight_tensors_[IN_QKV_WEIGHT_0],
          ...
          .contiguous();
  at_host_weight_tensors_[IN_QKV_WEIGHT_1] =
      torch::zeros({1}, torch::kFloat16).to(torch::kCPU);
  at_host_weight_tensors_[IN_QKV_WEIGHT_2] =
      torch::zeros({1}, torch::kFloat16).to(torch::kCPU);
  ...
  if (quantize_type_.compare("w8a8_dynamic") == 0) {
    ...
    at_host_weight_tensors_[IN_QKV_OFFSET_0] =
        torch::cat({at_host_weight_tensors_[IN_QKV_OFFSET_0],
                    at_host_weight_tensors_[IN_QKV_OFFSET_1],
                    at_host_weight_tensors_[IN_QKV_OFFSET_2]},
                   0)
            .contiguous()
            .view(-1);
    at_host_weight_tensors_[IN_QKV_OFFSET_1] =
    ...
    at_host_weight_tensors_[IN_ATTENTION_OUT_OFFSET] =
        at_host_weight_tensors_[IN_ATTENTION_OUT_OFFSET].contiguous().view(-1);

    at_host_weight_tensors_[IN_QKV_SCALE_0] =
        torch::cat({at_host_weight_tensors_[IN_QKV_SCALE_0],
                    at_host_weight_tensors_[IN_QKV_SCALE_1],
                    at_host_weight_tensors_[IN_QKV_SCALE_2]},
                   0)
            .contiguous()
            .view(-1);
    at_host_weight_tensors_[IN_QKV_SCALE_1] =
    ...
    at_host_weight_tensors_[IN_QKV_SCALE_2] =
        torch::zeros({1}, torch::kFloat16).to(torch::kCPU);
    at_host_weight_tensors_[IN_ATTENTION_OUT_SCALE] =
        at_host_weight_tensors_[IN_ATTENTION_OUT_SCALE].contiguous().view(-1);
  }
}

bool Qwen3MoeDecoderManualLoader::is_nz_format_tensor(int weight_index) {
  return weight_index == IN_MLP_GATEUP_WEIGHT_EXPERT ||
         weight_index == IN_MLP_DOWN_WEIGHT_EXPERT;
}
