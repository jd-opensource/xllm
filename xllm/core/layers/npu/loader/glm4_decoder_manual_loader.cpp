...
#include "glm4_decoder_manual_loader.h"
...
Glm4DecoderManualLoader::Glm4DecoderManualLoader(uint64_t weight_count,
...
void Glm4DecoderManualLoader::load_state_dict(const StateDict& state_dict) {
...
void Glm4DecoderManualLoader::merge_host_at_weights() {
  ...
  for (auto idx :
    ...
    at_host_weight_tensors_[idx] = make_zero_like(at_host_weight_tensors_[idx]);
  }
}

void Glm4DecoderManualLoader::verify_loaded_weights() const {
  for (const auto& [index, name] : WEIGHT_MAPPING) {
