...
==============================================================================*/
#include "deepseek_v2_decoder_manual_loader.h"
...
DeekseekV2DecoderManualLoader::DeekseekV2DecoderManualLoader(
...
void DeekseekV2DecoderManualLoader::initialize_tensors(
    const torch::TensorOptions& options) {
  torch::TensorOptions cpu_options = options.device(torch::kCPU);
  tensor_placeholder_ = torch::zeros({1}).to(cpu_options);
...
void DeekseekV2DecoderManualLoader::load_state_dict(
...
void DeekseekV2DecoderManualLoader::verify_loaded_weights(
...
int DeekseekV2DecoderManualLoader::extract_expert_index(
...
void DeekseekV2DecoderManualLoader::init_device_at_weights() {
...
void DeekseekV2DecoderManualLoader::process_expert_weights(
    const StateDict& state_dict,
    const std::string& name,
...
void DeekseekV2DecoderManualLoader::initialize_weight_tensors(
    const torch::TensorOptions& options) {
  torch::TensorOptions cpu_options = options.device(torch::kCPU);
  for (int i = 0; i < weight_count_; ++i) {
    at_weight_tensors_[i] = torch::zeros({1}).to(options);
    at_host_weight_tensors_[i] = torch::zeros({1}).to(cpu_options);
  }
...
void DeekseekV2DecoderManualLoader::convert_offsets_to_int8() {
...
void DeekseekV2DecoderManualLoader::handle_device_specific_bias() {
...
std::string DeekseekV2DecoderManualLoader::extract_endswith(
...
torch::Tensor DeekseekV2DecoderManualLoader::get_sharded_tensor(
    const StateDict& state_dict,
    const std::string& name,
    int dim) {
...
int DeekseekV2DecoderManualLoader::get_mapped_index(
    const std::string& name,
...
void DeekseekV2DecoderManualLoader::squeeze_experts_weights() {
...
void DeekseekV2DecoderManualLoader::process_general_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
...
void DeekseekV2DecoderManualLoader::process_mlp_common_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
  const int index = get_mapped_index(name, WEIGHT_MAPPING_W8A8);
...
void DeekseekV2DecoderManualLoader::merge_experts_weights() {
  ...
  if (quantize_type_ == "w8a8_dynamic") {
    at_host_weight_tensors_[IN_MLP_DOWN_OFFSET_EXPERT] = merge_experts_weights(
        experts_weights_["down_proj.weight_offset"], torch::kCPU);
    at_host_weight_tensors_[IN_MLP_DOWN_SCALE_EXPERT] = merge_experts_weights(
        experts_weights_["down_proj.weight_scale"], torch::kCPU);
  }
}

torch::Tensor DeekseekV2DecoderManualLoader::merge_experts_weights(
...
void DeekseekV2DecoderManualLoader::process_shared_expert_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
  torch::Tensor tmp_tensor;
...
void DeekseekV2DecoderManualLoader::set_kv_weight(
    const StateDict& state_dict,
...
void DeekseekV2DecoderManualLoader::preprocess_linear_for_rope() {
...
torch::Tensor DeekseekV2DecoderManualLoader::view_tensor(
    torch::Tensor weight,
    const std::string& name,
    bool pre_view) {
...
torch::Tensor DeekseekV2DecoderManualLoader::trans_rope_weight(
    torch::Tensor weight) {
...
void DeekseekV2DecoderManualLoader::initialize_device_expert_list(
    int num_device,
...
torch::Tensor DeekseekV2DecoderManualLoader::convert_fp16_to_int64(
...
void DeekseekV2DecoderManualLoader::convert_descaled_weights_to_float() {
...
void DeekseekV2DecoderManualLoader::reserve_experts_weights(
...
std::string DeekseekV2DecoderManualLoader::get_expert_shm_key(
    int32_t layer_id,
...
void DeekseekV2DecoderManualLoader::merge_shared_experts_weights() {
...
void DeekseekV2DecoderManualLoader::merge_loaded_weights() {
...
void DeekseekV2DecoderManualLoader::merge_and_move_pinned_host() {
  merge_host_at_weights();
  init_weight_slices();
  copy_weights_to_pinned_host();
}

void DeekseekV2DecoderManualLoader::copy_weights_to_pinned_host() {
  ...
  for (size_t i = 0; i < weight_slices_.size(); ++i) {
    ...
    void* dst_ptr = static_cast<char*>(host_pinned_storage_) +
                    static_cast<ptrdiff_t>(slice.offset);

    if (is_nz_format_tensor(i)) {
      auto tmp_tensor =
          at_npu::native::npu_format_cast(host_tensor.to(device_), 29);
      const void* src_ptr = tmp_tensor.data_ptr();
      auto stream = c10_npu::getCurrentNPUStream();
      aclrtMemcpyAsync(dst_ptr,
      ...
      std::memcpy(dst_ptr, host_tensor.data_ptr(), slice.bytes);
    }
...
bool DeekseekV2DecoderManualLoader::is_nz_format_tensor(int weight_index) {
  if (layer_id_ >= prefill_firstKDenseReplace_) {
    if (weight_index == IN_MLP_GATEUP_WEIGHT_EXPERT) {
      return true;
    } else if (weight_index == IN_MLP_DOWN_WEIGHT_EXPERT) {
#if defined(USE_A3)
      return false;
#else
      if (decode_isBF16_) {
...
void DeekseekV2DecoderManualLoader::copy_weights_to_device() {
  CHECK_EQ(weight_slices_.size(), at_host_weight_tensors_.size())
      << "weight_slices_ size and at_host_weight_tensors_ size mismatch.";
  ...
  for (size_t i = 0; i < weight_slices_.size(); ++i) {
    ...
    void* dst_ptr = static_cast<char*>(device_storage_) +
                    static_cast<ptrdiff_t>(slice.offset);
    ...
    if (is_nz_format_tensor(i)) {
      ...
      const void* src_ptr = tmp_tensor.data_ptr();
      auto stream = c10_npu::getCurrentNPUStream();
      auto err = aclrtMemcpyAsync(dst_ptr,
                                  slice.bytes,
                                  src_ptr,
                                  slice.bytes,
...
void DeekseekV2DecoderManualLoader::merge_host_at_weights() {
  if (quantize_type_ == "w8a8_dynamic") {
    if (prefill_isBF16_) {
