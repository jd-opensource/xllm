...
DeepseekV2DecoderManualLoader::DeepseekV2DecoderManualLoader(
...
void DeepseekV2DecoderManualLoader::initialize_tensors(
...
void DeepseekV2DecoderManualLoader::load_state_dict(
...
void DeepseekV2DecoderManualLoader::verify_loaded_weights(
...
int DeepseekV2DecoderManualLoader::extract_expert_index(
...
void DeepseekV2DecoderManualLoader::process_expert_weights(
    const StateDict& state_dict,
    const std::string& name,
...
void DeepseekV2DecoderManualLoader::initialize_weight_tensors(
    const torch::TensorOptions& options) {
...
void DeepseekV2DecoderManualLoader::convert_offsets_to_int8() {
...
void DeepseekV2DecoderManualLoader::handle_device_specific_bias() {
...
std::string DeepseekV2DecoderManualLoader::extract_endswith(
...
torch::Tensor DeepseekV2DecoderManualLoader::get_sharded_tensor(
    const StateDict& state_dict,
    const std::string& name,
    int dim) {
...
int DeepseekV2DecoderManualLoader::get_mapped_index(
    const std::string& name,
...
void DeepseekV2DecoderManualLoader::squeeze_experts_weights() {
...
void DeepseekV2DecoderManualLoader::process_general_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
...
void DeepseekV2DecoderManualLoader::process_mlp_common_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
  const int index = get_mapped_index(name, WEIGHT_MAPPING_W8A8);
...
void DeepseekV2DecoderManualLoader::merge_experts_weights() {
...
#else
  // Keep non-A3 path explicit: MTP transpose route is not enabled yet.
  torch::Tensor mlp_down_weight =
      merge_experts_weights(experts_weights_["down_proj.weight"],
                            /*transpose=*/false);
  at_host_weight_tensors_[IN_MLP_DOWN_WEIGHT_EXPERT] =
...
torch::Tensor DeepseekV2DecoderManualLoader::merge_experts_weights(
...
void DeepseekV2DecoderManualLoader::process_shared_expert_weights(
    const StateDict& state_dict,
    const std::string& name,
    const torch::Tensor& tensor) {
  torch::Tensor tmp_tensor;
...
void DeepseekV2DecoderManualLoader::set_kv_weight(
    const StateDict& state_dict,
...
void DeepseekV2DecoderManualLoader::preprocess_linear_for_rope() {
...
torch::Tensor DeepseekV2DecoderManualLoader::view_tensor(
    torch::Tensor weight,
    const std::string& name,
    bool pre_view) {
...
torch::Tensor DeepseekV2DecoderManualLoader::trans_rope_weight(
    torch::Tensor weight) {
...
void DeepseekV2DecoderManualLoader::initialize_device_expert_list(
    int num_device,
...
torch::Tensor DeepseekV2DecoderManualLoader::convert_fp16_to_int64(
...
void DeepseekV2DecoderManualLoader::convert_descaled_weights_to_float() {
...
void DeepseekV2DecoderManualLoader::reserve_experts_weights(
...
std::string DeepseekV2DecoderManualLoader::get_expert_shm_key(
    int32_t layer_id,
...
void DeepseekV2DecoderManualLoader::merge_shared_experts_weights() {
...
void DeepseekV2DecoderManualLoader::merge_host_at_weights() {
  if (quantize_type_ == "w8a8_dynamic") {
    if (prefill_isBF16_) {
...
bool DeepseekV2DecoderManualLoader::is_nz_format_tensor(int weight_index) {
  if (layer_id_ >= prefill_firstKDenseReplace_) {
