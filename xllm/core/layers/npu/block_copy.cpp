...
std::shared_ptr<BlockCopyImpl> create_block_copy_layer(
    const ModelContext& context) {
  return std::make_shared<BlockCopyImpl>(context);
}

BlockCopyImpl::BlockCopyImpl(const ModelContext& context) : ATBBase(context) {
...
int64_t BlockCopyImpl::init_layer() {
  ATBBase::name_ = "block_copy_layer";
  model_name_ = "llm";
  runTaskFunc_ = std::bind(&BlockCopyImpl::run_task,
                           this,
                           std::placeholders::_1,
                           std::placeholders::_2);
...
BlockCopy::BlockCopy(const ModelContext& context)
    : ModuleHolder(create_block_copy_layer(context)) {}
