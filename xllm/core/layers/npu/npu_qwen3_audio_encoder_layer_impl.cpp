...
void NpuQwen3AudioEncoderLayerImpl::param_from_args(
  ...
  if (padding_heads) {
    LOG(FATAL) << "You are running qwen3 audio encoder with " << param.worldSize
               ...
               << " the model could only support 1, 2, 4 cards in tensor "
                  "parallel currently";
  }
  ...
  std::optional<uint32_t> optionalValue = args.mm_audio_num_attention_heads();
  param.numKeyValueHeadsPerRank =
      static_cast<uint32_t>(optionalValue.value()) / param.worldSize;
...
NpuQwen3AudioEncoderLayerImpl::NpuQwen3AudioEncoderLayerImpl(
    ...
    : BaseLayer(context), model_args_(context.get_model_args()) {
  ...
  loader_ = std::make_unique<Qwen3AudioEncoderLoader>(WEIGHT_COUNT_PER_LAYER,
                                                      context);
}
...
torch::Tensor NpuQwen3AudioEncoderLayerImpl::forward(
  ...
  build_node_variant_pack(
      encode_node_, x, cu_seqlen, cu_seqlen_vec, input_params, true);
  // mstxRangeEnd(id);
