...
int64_t ResolveOneRecBatchSize(const ForwardInput& input) {
  const auto* onerec_params =
      input.onerec_xattention_params() != nullptr
          ? static_cast<const OneRecInput*>(input.onerec_xattention_params())
          : input.onerec_params();
  if (onerec_params != nullptr && onerec_params->bs > 0) {
    return onerec_params->bs;
  }
  return std::max<int64_t>(input.meta.num_sequences, 1);
}
  ...
  if (is_prefill) {
    if (is_decoder_) {
      ...
      build_encoder_node_variant_pack(
          prefill_node_, x, attn_mask, input, true, node_id);
      st = execute_node(prefill_node_, node_id, event, event_flag);
      LOG_IF(FATAL, st != 0)
          << model_name_
...
int32_t NpuOneRecBlockLayerImpl::setup_common_decoder_tensors(
  ...
  if (attn_mask.defined()) {
    ...
    if (!param.isPrefill && param.isDecoder) {
      prefill_attn_mask_ = NormalizeOneRecDecodeFasMask(
          prefill_attn_mask_, ResolveOneRecBatchSize(input));
    }
  ...
  if (!param.enableOneRecPrefillOnly &&
      input.attention.device.block_tables.defined()) {
    node.variantPack.inTensors.at(idx) =
  ...
  if (!param.enableOneRecPrefillOnly &&
      input.attention.device.new_cache_slots.defined()) {
