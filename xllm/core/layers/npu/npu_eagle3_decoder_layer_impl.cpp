...
void NpuEagle3DecoderLayerImpl::param_from_args(
  ...
  param.numAttentionHeadsPerRank = model_args->n_heads() / param.worldSize;
  // param.hiddenSizePerAttentionHead = model_args->hidden_size() /
  // model_args->n_heads();
  ...
  // param.numKeyValueHeadsPerRank = model_args->n_kv_heads();
  std::optional<long int> optionalValue = model_args->n_kv_heads();
  param.numKeyValueHeadsPerRank =
      static_cast<int>(optionalValue.value()) / param.worldSize;
  ;
  // param.numKeyValueHeadsPerRank = static_cast<int>(model_args->n_kv_heads());
