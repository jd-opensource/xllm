...
void NpuGlm4MoeDecoderImpl::param_from_args(
    ...
    bool is_prefill) {
  initialize_basic_parameters(param, model_args, parallel_args, is_prefill);
...
void NpuGlm4MoeDecoderImpl::initialize_basic_parameters(
  ...
  param.enableAclGraphPagedAttention =
      FLAGS_enable_graph && !is_prefill && model_args->n_layers() > 1;
  ...
  param.useQKNorm = model_args->use_qk_norm();
  if (model_args->use_qk_norm()) {
    WEIGHT_COUNT_PER_LAYER = 70;
  }
  param.hiddenSizePerAttentionHead = model_args->head_dim();
...
void NpuGlm4MoeDecoderImpl::initialize_mlp_parameters(
  ...
  param.numOfExperts = model_args->num_experts();
  param.numOfDeviceExperts = model_args->num_experts();
