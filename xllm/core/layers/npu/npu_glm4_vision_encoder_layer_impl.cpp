...
void NpuGlm4VisionEncoderLayerImpl::param_from_args(
  ...
  param.numAttentionHeadsPerRank =
      model_args->mm_num_attention_heads() / param.worldSize;
  param.hiddenSizePerAttentionHead =
      model_args->mm_hidden_size() / model_args->mm_num_attention_heads();
  std::optional<long int> optionalValue = model_args->mm_num_attention_heads();
