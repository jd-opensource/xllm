...
bool FusedMoEImpl::can_use_ep2_dispatch_combine(
  ...
  if (mode == 1) {
    if (forward_input.enable_graph && !dispatch_ffn_combine_prepared_) {
      return false;
    }
  ...
  if (!all_dp_ranks_are_decode(forward_input.parallel)) {
    return false;
  }
  if (mode == 2) {
    if (forward_input.enable_graph && !dispatch_gmm_combine_decode_prepared_) {
      return false;
    }
    return is_w8a8_dynamic_quant_method(resolved_moe_quant_method_) &&
...
torch::Tensor FusedMoEImpl::forward_with_selected_experts_ep2(
  ...
  if (parallel_args_.dp_size() > 1 &&
      !forward_input.parallel.dp_global_token_nums.empty()) {
...
torch::Tensor FusedMoEImpl::forward_with_selected_experts(
  ...
  if (can_use_ep2_dispatch_combine(forward_input, moe_input)) {
    std::optional<torch::Tensor> shared_output = std::nullopt;
    if (n_shared_experts_ > 0) {
    ...
    auto output = forward_with_selected_experts_ep2(
        moe_input, selected_topk_weights, selected_topk_ids, forward_input);
    if (shared_output.has_value()) {
  ...
  if (should_gather_dp_inputs_for_moe()) {
    moe_input =
        parallel_state::gather(moe_input,
                               parallel_args_.dp_local_process_group_,
