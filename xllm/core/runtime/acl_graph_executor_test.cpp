...
class AclGraphExecutorTest : public ::testing::Test {
 protected:
  ...
  void TearDown() override {
    sequences_.clear();  // Clear all sequences
    model_.reset();
  ...
  std::unique_ptr<SimpleCausalLM> model_;
...
TEST_F(AclGraphExecutorTest, DifferentBatchSizes) {
  ...
  for (auto batch_size : batch_sizes) {
    ...
    for (uint32_t i = 0; i < batch_size; ++i) {
      ...
      // Allocate blocks and configure sequence
      auto allocated_blocks =
          block_manager_->allocate(2);  // Allocate more blocks for safety
      sequence.add_kv_blocks(allocated_blocks);
