...
std::optional<ForwardOutput> LLMWorkerImpl::step(
  ...
  // should be in same prefill stage, so, to judge forward_type,
  // just use micro batch 0 here
  if (options_.enable_speculative_decode() && !is_spec_draft_) {
    CHECK_EQ(input_params_micro_batches[0].batch_forward_type.is_mixed(),
             false);
    if (!input_params_micro_batches[0].batch_forward_type.is_decode()) {
