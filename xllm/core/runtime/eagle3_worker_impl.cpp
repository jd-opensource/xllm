...
runtime::Options Eagle3MainOptions(const runtime::Options& options) {
  auto opts = options;
  opts.enable_schedule_overlap(false).enable_graph_aux_hidden_states(true);
  return opts;
}

runtime::Options Eagle3DraftOptions(const runtime::Options& options) {
  auto opts = options;
...
Eagle3WorkerImpl::Eagle3WorkerImpl(const ParallelArgs& parallel_args,
    ...
    : SpeculativeWorkerImpl(parallel_args,
                            device,
                            Eagle3MainOptions(options),
                            Eagle3DraftOptions(options)) {}
