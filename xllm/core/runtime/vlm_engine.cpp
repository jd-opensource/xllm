...
bool VLMEngine::allocate_kv_cache(const Engine::KVCacheCapacity& kv_cache_cap) {
  ...
  options.num_blocks(kv_cache_cap.n_blocks)
      .host_num_blocks(0)  // no host cache for vlm engine
...
ForwardOutput VLMEngine::step(std::vector<Batch>& batch) {
  ...
  mstxRangeId prepare_inputs_id =
      mstxRangeStartA("engine prepare input", nullptr);
  auto batched_raw_forward_inputs = prepare_inputs(batch);
  mstxRangeEnd(prepare_inputs_id);
  ...
  for (auto worker_rank = 0; worker_rank < worker_clients_num_;
    ...
    if (result.has_value()) {
      ...
      if (result.value().src_seq_idxes.size() == 0) {
        ...
        mstxRangeId prepare_inputs_id =
            mstxRangeStartA("process_sample_output", nullptr);
