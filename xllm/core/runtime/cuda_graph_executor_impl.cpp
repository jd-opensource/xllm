...
bool CudaGraphPersistentParam::can_use_llm_decode_fast_path(
  ...
  if (!input.meta.batch_forward_type.is_decode() || is_rec_multi_round_mode() ||
      input.has_llmrec_params() || input.embedding.input_embedding.defined()) {
    return false;
  }
  return is_cuda_contiguous_int32_tensor(input.token_ids) &&
         is_cuda_contiguous_int32_tensor(input.positions) &&
         is_cuda_contiguous_int32_tensor(
             input.attention.device.new_cache_slots) &&
         is_cuda_contiguous_int32_tensor(input.attention.device.kv_seq_lens) &&
         is_cuda_contiguous_int32_tensor(
...
void CudaGraphPersistentParam::update_llm_decode_metadata_fast_path(
  ...
  const int64_t actual_indices_size =
      input.attention.device.paged_kv_indices.size(0);
  xllm::kernel::cuda::LlmDecodeMetadataUpdateParams update_params{
      .src_tokens = input.token_ids.data_ptr<int32_t>(),
      .src_positions = input.positions.data_ptr<int32_t>(),
      .src_new_cache_slots =
          input.attention.device.new_cache_slots.data_ptr<int32_t>(),
      .src_kv_seq_lens = input.attention.device.kv_seq_lens.data_ptr<int32_t>(),
      .src_paged_kv_indptr =
          input.attention.device.paged_kv_indptr.data_ptr<int32_t>(),
      .src_paged_kv_indices =
          input.attention.device.paged_kv_indices.data_ptr<int32_t>(),
      .src_paged_kv_last_page_len =
          input.attention.device.paged_kv_last_page_len.data_ptr<int32_t>(),
...
std::optional<ForwardInput> CudaGraphPersistentParam::update(
    const ForwardInput& input,
  ...
  if (return_capture_input) {
    CHECK_GT(padded_num_tokens, 0)
        << "padded_num_tokens must be > 0 when return_capture_input is true";
    input_for_capture = input;
  ...
  // Build attn_metadata with original model_input. So we can set actual
  ...
  auto build_capture_input_if_needed = [&]() -> std::optional<ForwardInput> {
    if (!return_capture_input) {
      return std::nullopt;
    }
    CHECK(input_for_capture.has_value())
        << "input_for_capture should be initialized when "
           "return_capture_input is true";
    ...
    if (!input.embedding.linear_state_ids.empty()) {
      input_for_capture->embedding.linear_state_ids =
          input.embedding.linear_state_ids;
      input_for_capture->embedding.linear_state_indices =
          persistent_linear_state_indices(input.meta.num_sequences);
    }
  ...
  const int64_t actual_batch_size = input.meta.num_sequences;
  const bool use_llm_decode_fast_path = can_use_llm_decode_fast_path(input);

  // Copy data from input parameters to persistent graph tensors
  if (use_llm_decode_fast_path) {
    VLOG(kGraphExecutorLogVerboseLevel)
        << "use fast path for LLM decode metadata update";
    update_llm_decode_metadata_fast_path(
        input, padded_num_tokens, actual_batch_size, actual_num_tokens);
  } else {
    VLOG(kGraphExecutorLogVerboseLevel)
  ...
  if (!is_rec_multi_round_mode() && !use_llm_decode_fast_path) {
    ...
    VLOG(kGraphExecutorLogVerboseLevel)
        ...
        << input.attention.device.q_seq_lens.sizes() << ", dst slice shape=["
        << actual_batch_size + 1 << "]";
    q_seq_lens_.slice(/*dim=*/0, /*start=*/0, /*end=*/actual_batch_size + 1)
        .copy_(input.attention.device.q_seq_lens, /*non_blocking=*/true);

    VLOG(kGraphExecutorLogVerboseLevel)
        << "copy_ kv_seq_lens: src shape="
        << input.attention.device.kv_seq_lens.sizes() << ", dst slice shape=["
        << actual_batch_size + 1 << "]";
    kv_seq_lens_.slice(/*dim=*/0, /*start=*/0, /*end=*/actual_batch_size + 1)
        .copy_(input.attention.device.kv_seq_lens, /*non_blocking=*/true);

    VLOG(kGraphExecutorLogVerboseLevel)
        << "copy_ new_cache_slots: src shape="
        << input.attention.device.new_cache_slots.sizes()
        << ", dst slice shape=[" << actual_num_tokens << "]";
    persistent_new_cache_slots_
        .slice(/*dim=*/0, /*start=*/0, /*end=*/actual_num_tokens)
        .copy_(input.attention.device.new_cache_slots,
               /*non_blocking=*/true);
    if (padded_num_tokens > actual_num_tokens) {
      persistent_new_cache_slots_
          .slice(/*dim=*/0,
  ...
  if (!is_rec_multi_round_mode() && !input.embedding.linear_state_ids.empty()) {
    if (input.embedding.linear_state_indices.defined()) {
      persistent_linear_state_indices_
          .slice(/*dim=*/0, /*start=*/0, /*end=*/actual_batch_size)
          .copy_(input.embedding.linear_state_indices, /*non_blocking=*/true);
    } else {
      persistent_linear_state_indices_
          .slice(/*dim=*/0, /*start=*/0, /*end=*/actual_batch_size)
          .copy_(torch::tensor(input.embedding.linear_state_ids, torch::kInt)
                     .to(device_),
                 /*non_blocking=*/true);
    }
  }
  ...
  const int64_t actual_block_table_batch =
      is_rec_multi_round_mode() ? input.attention.device.block_tables.size(0)
                                : actual_batch_size;
  const int64_t actual_block_table_len =
      input.attention.device.block_tables.size(1);
  ...
  VLOG(kGraphExecutorLogVerboseLevel)
      ...
      << input.attention.device.block_tables.sizes()
      << ", dst slice shape=" << slice_persistent_block_tables.sizes();
  slice_persistent_block_tables.copy_(input.attention.device.block_tables,
                                      /*non_blocking=*/true);
  ...
  const auto& embedding = input.embedding.input_embedding;
  ...
  const bool is_decode_with_llmrec =
      input.meta.batch_forward_type.is_decode() && input.has_llmrec_params();
  ...
  if (use_two_stage_decode) {
    if (input.attention.device.q_seq_lens.defined() &&
        input.attention.device.q_seq_lens.numel() > 0) {
      const int64_t q_numel = input.attention.device.q_seq_lens.numel();
    ...
    if (input.attention.device.kv_seq_lens.defined() &&
        input.attention.device.kv_seq_lens.numel() > 0) {
      const int64_t kv_numel = input.attention.device.kv_seq_lens.numel();
    ...
    const auto& llmrec_params = *input.llmrec_params();
  ...
  if (use_llm_decode_fast_path) {
    ...
    CHECK(input.attention.device.paged_kv_indptr.defined())
        << "paged_kv_indptr should not be null";
    VLOG(kGraphExecutorLogVerboseLevel)
        << "copy_ paged_kv_indptr: src shape="
        << input.attention.device.paged_kv_indptr.sizes()
    ...
    persistent_paged_kv_indptr_
        ...
        .copy_(input.attention.device.paged_kv_indptr, /*non_blocking=*/true);
    CHECK(input.attention.device.paged_kv_indices.defined())
        << "paged_kv_indices should not be null";
    const int64_t actual_indices_size =
        input.attention.device.paged_kv_indices.size(0);
    VLOG(kGraphExecutorLogVerboseLevel)
        << "copy_ paged_kv_indices: src shape="
        << input.attention.device.paged_kv_indices.sizes()
        << ", dst slice shape=[" << actual_indices_size << "]";
    persistent_paged_kv_indices_
        .slice(/*dim=*/0,
               /*start=*/0,
               /*end=*/actual_indices_size)
        .copy_(input.attention.device.paged_kv_indices,
               /*non_blocking=*/true);
    CHECK(input.attention.device.paged_kv_last_page_len.defined())
        << "paged_kv_last_page_len should not be null";
    VLOG(kGraphExecutorLogVerboseLevel)
        << "copy_ paged_kv_last_page_len: src shape="
        << input.attention.device.paged_kv_last_page_len.sizes()
...
bool CudaGraph::capture(CausalLM* model,
  ...
  // Use the returned ForwardInput for graph capture
  ...
  auto graph_input_opt =
      persistent_param_.update(input,
                               k_cache,
                               v_cache,
                               padded_num_tokens_,
  ...
  CHECK(graph_input_opt.has_value())
      << "update() should return ForwardInput when return_capture_input=true";
  ...
  if (is_piecewise_) {
    ...
    ForwardInput warmup_input = graph_input_opt.value();
    model->forward(warmup_input, kv_cache);
...
ModelOutput CudaGraph::replay(const ForwardInput& input,
                              std::vector<KVCache>& kv_cache) {
  const torch::Tensor& tokens = input.token_ids;
  const uint32_t actual_num_tokens = tokens.size(0);
  ...
  if (is_piecewise_) {
    ...
    auto updated_input_opt =
        persistent_param_.update(input,
                                 k_cache,
                                 v_cache,
                                 padded_num_tokens_,
                                 /*return_capture_input=*/true);
    CHECK(updated_input_opt.has_value())
        << "update() should return ForwardInput for piecewise replay";

    const auto& updated_input = updated_input_opt.value();
    ...
    CHECK(updated_input.attn_metadata->plan_info)
        << "plan_info is required for piecewise replay";

    VLOG(kGraphExecutorLogVerboseLevel)
        << "CudaGraph::replay() piecewise replay with uri="
        << updated_input.attn_metadata->plan_info->uri << ", plan_info.defined="
        << updated_input.attn_metadata->plan_info->plan_info.defined();
    ...
    replay_params.plan_info = updated_input.attn_metadata->plan_info->plan_info;
    replay_params.q_cu_seq_lens = updated_input.attn_metadata->q_cu_seq_lens;
    replay_params.kv_cu_seq_lens = updated_input.attn_metadata->kv_cu_seq_lens;
...
ModelOutput CudaGraphExecutorImpl::run(const ForwardInput& input,
                                       std::vector<KVCache>& kv_caches) {
  const torch::Tensor& tokens = input.token_ids;
  const bool is_prefill = input.meta.batch_forward_type.is_prefill();
  const bool is_decode = input.meta.batch_forward_type.is_decode();
  ...
  if (is_prefill && enable_prefill_piecewise_graph_) {
    ...
    if (it != prefill_graphs_.end()) {
      ...
      auto result = it->second->replay(input, kv_caches);
    ...
    if (capture_success) {
      ...
      // subsequent requests.
      auto result =
          prefill_graphs_[bucket_num_tokens]->replay(input, kv_caches);
      return attach_aux_hidden_states_if_needed(result.hidden_states, n_tokens);
    }
  ...
  if (is_decode) {
    ...
    const bool seq_len_supported = input.meta.kv_max_seq_len <= max_seq_len;
    ...
    if (capture_success) {
      ...
      // Run replay after capture so first request uses same execution path as
      // subsequent requests.
      auto result = graphs_[bucket_num_tokens]->replay(input, kv_caches);
      return attach_aux_hidden_states_if_needed(result.hidden_states, n_tokens);
    }
