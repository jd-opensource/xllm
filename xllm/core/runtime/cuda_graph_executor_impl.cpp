...
CudaGraphPersistentParam::CudaGraphPersistentParam(
  ...
  const int64_t max_seqs_per_batch =
      options.max_seqs_per_batch() * options_.beam_width();
...
std::optional<ModelInputParams> CudaGraphPersistentParam::update(
  ...
  const int64_t num_sequences = params.num_sequences;
  int32_t actual_batch_size;
  if (params.has_llmrec_params()) {
    actual_batch_size = params.llmrec_params()->batch_size;
  } else {
    actual_batch_size = params.num_sequences;
  }
  ...
  if (!is_rec_multi_round_mode()) {
    ...
    VLOG(kGraphExecutorLogVerboseLevel)
        ...
        << ", dst slice shape=[" << num_sequences + 1 << "]";
    q_seq_lens_.slice(/*dim=*/0, /*start=*/0, /*end=*/num_sequences + 1)
        .copy_(params.q_seq_lens, /*non_blocking=*/true);

    VLOG(kGraphExecutorLogVerboseLevel)
        << "copy_ kv_seq_lens: src shape=" << params.kv_seq_lens.sizes()
        << ", dst slice shape=[" << num_sequences + 1 << "]";
    kv_seq_lens_.slice(/*dim=*/0, /*start=*/0, /*end=*/num_sequences + 1)
  ...
  if (is_rec_multi_round_mode()) {
    ...
    slice_persistent_block_tables =
        persistent_block_tables_
            .slice(/*dim=*/0, /*start=*/0, /*end=*/num_sequences)
            .slice(/*dim=*/1, /*start=*/0, /*end=*/actual_block_table_len);
  }
  ...
  attn_metadata->kv_seq_lens =
      torch::diff(kv_seq_lens(/*num_sequences=*/num_sequences + 1));
  ...
  attn_metadata->paged_kv_indptr = persistent_paged_kv_indptr(num_sequences);
  attn_metadata->paged_kv_indices =
      persistent_paged_kv_indices(actual_indices_size);
  attn_metadata->paged_kv_last_page_len =
      persistent_paged_kv_last_page_len(num_sequences);
  // qo_indptr is q_cu_seq_lens in GPU Model.
  attn_metadata->qo_indptr = persistent_decode_qo_indptr(num_sequences);
