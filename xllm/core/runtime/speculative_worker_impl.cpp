...
#if defined(USE_MLU)
    // MTP: Add an extra draft step to match the KV cache patch in normal decode
    // path. This ensures consistent execution steps across all DP workers.
    if (options_.num_speculative_tokens() == 1) {
      auto patch_future = draft_impl_->step_async(input);
...
#if defined(USE_MLU)
  patch_draft_kv_cache_for_accepted_tokens(input, val_output);
#endif
...
#if defined(USE_MLU)
void SpeculativeWorkerImpl::patch_draft_kv_cache_for_accepted_tokens(
    const ForwardInput& input,
  ...
  new_token_ids.reserve(batch_size);
  new_positions.reserve(batch_size);
  new_token_slot_ids.reserve(batch_size);
  block_tables_vec.reserve(batch_size);
  embedding_ids.reserve(batch_size);

  int32_t kv_max_seq_len = 0;
  const int32_t block_size = options_.block_size();
  // In DP mode, all sequences must participate to ensure consistent execution
  // across workers, even if the computation is wasted for rejected tokens.
  for (int32_t seq_id = 0; seq_id < batch_size; ++seq_id) {
    new_token_ids.emplace_back(accepted_tokens[seq_id][0].item<int32_t>());
