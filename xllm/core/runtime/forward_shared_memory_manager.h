...
class ForwardSharedMemoryManager : public SharedMemoryManager {
 public:
  ...
  int pb_write(const PbType* pb_data) {
  ...
  int raw_input_write(const std::vector<RawForwardInput>& inputs);
  void raw_input_read(std::vector<ForwardInput>& inputs);
  int raw_output_write(const torch::Tensor& next_tokens,
