...
INLINE void write_tensor(char*& buffer, const torch::Tensor& tensor) {
  ...
  // write dtype
  int8_t dtype = static_cast<int8_t>(contig_tensor.scalar_type());
  write_data(buffer, dtype);
  // write ndim
  const uint64_t ndim = contig_tensor.dim();
  ...
  if (data_bytes > 0) {
    std::memcpy(buffer, contig_tensor.data_ptr(), data_bytes);
    buffer += data_bytes;
  }
}
