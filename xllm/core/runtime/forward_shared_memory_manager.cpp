  ...
  auto read_mm_data =
      is_mm_item
          ? [](const char*& data_buffer,
               MMData& mm_data_item,
               const char*& data_device_buffer) {
              read_mm_data_items(data_buffer, mm_data_item, data_device_buffer);
            }
          : [](const char*& data_buffer,
               MMData& mm_data_item,
               const char*& data_device_buffer) {
              read_mm_data_dict(data_buffer, mm_data_item, data_device_buffer);
            };
...
inline void read_mm_batch_data(ReadContext& context,
  ...
  for (size_t i = 0; i < mm_data_num; ++i) {
    if (is_mm_item) {
      read_mm_data_items(context, mm_data);
    } else {
      read_mm_data_dict(context, mm_data);
    }
    vec.push_back(std::move(mm_data));
  }
