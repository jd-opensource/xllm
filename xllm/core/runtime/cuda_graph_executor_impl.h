...
class CudaGraphPersistentParam {
 ...
 private:
  bool can_use_llm_decode_fast_path(const ForwardInput& forward_input) const;
  void update_llm_decode_metadata_fast_path(const ForwardInput& forward_input,
...
class CudaGraph {
 public:
  ...
  ModelOutput replay(const ForwardInput& forward_input,
                     std::vector<KVCache>& kv_cache);
