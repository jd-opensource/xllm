...
bool AclGraph::capture(CausalLM* model,
  ...
  LOG(INFO) << "capture begin, batch_size: " << batch_size_ << std::endl;
...
torch::Tensor AclGraph::replay(const torch::Tensor& tokens,
  ...
  // Get actual batch size from tokens shape
  // tokens.size(0) is the total number of decode tokens
  // We need to divide by num_decoding_tokens to get actual batch size
  // For now, we'll use the batch_size from params as it should be the actual
  // batch size
  const uint32_t actual_batch_size = batch_size;
...
torch::Tensor AclGraphExecutorImpl::run(const torch::Tensor& tokens,
  ...
  if (it != graphs_.end()) {
    ...
    if (seq_len_supported && same_num_decoding_tokens) {
      // Create and capture graph for this bucket size with NPUGraph mempool
      ...
      if (capture_success) {
        ...
        LOG(ERROR) << "Failed to capture ACL graph for bucket size: "
                   << bucket_size;
