...
MMDict VlmExecutorImpl::encode(const ForwardInput& forward_input) {
  return dynamic_cast<CausalVLM*>(model_)->encode(forward_input);
}

ModelOutput VlmExecutorImpl::run(const ForwardInput& forward_input,
                                 std::vector<KVCache>& kv_caches) {
  torch::NoGradGuard no_grad;
  ForwardInput model_input = forward_input;
