...
bool RecEngine::RecMultiRoundEnginePipeline::init_model_workers(
  ...
  // process_group.*. Multi-card NPU keeps the direct HCCL path, and other local
  // backends continue to use create_local_process_groups().
  if (world_size == 1 && devices[0].is_privateuseone()) {
    std::string host;
    int port;
