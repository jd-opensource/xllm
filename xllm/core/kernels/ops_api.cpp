...
torch::Tensor causal_conv1d_update(CausalConv1dUpdateParams& params) {
  ...
  auto x_fp16 = x_work.to(torch::kFloat16);
  auto weight_fp16 = weight_work.to(torch::kFloat16);
  auto conv_state_fp16 = conv_state_work.to(torch::kFloat16).clone();
  auto bias_fp16 = bias_work.to(torch::kFloat16);

  auto y_fp16 = torch::empty({x_work.size(0), dim}, x_fp16.options());
  auto cu_b = cu_seqlens.slice(0, 0, 2);

  for (int64_t b = 0; b < batch; ++b) {
    auto x_b = x_fp16.slice(0, b, b + 1);
    ...
    y_fp16.slice(0, b, b + 1).copy_(y_b);
  }

  params.conv_state.copy_(conv_state_fp16.to(original_dtype));
  auto y = y_fp16.to(original_dtype);
