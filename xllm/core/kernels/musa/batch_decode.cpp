...
void batch_decode(torch::Tensor& float_workspace_buffer,
  ...
  std::tie(output,
      ...
      FunctionFactory::get_instance().mate_func().call(
          ...
          std::nullopt,     // "Tensor? cu_seqlens_q = None,"
          std::nullopt,     // "Tensor? cu_seqlens_k = None,"
          std::nullopt,     // "Tensor? cu_seqlens_k_new = None,"
          std::nullopt,     // "Tensor? seqused_q = None,"
          kv_seq_lens,      // "Tensor? seqused_k = None,"
          1,                // "int? max_seqlen_q = None,"
          std::nullopt,     // "int? max_seqlen_k = None,"
          block_table,      // "Tensor? page_table = None,"
