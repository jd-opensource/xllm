...
void batch_prefill(const std::string& uri,
  ...
  if (backend == "fa2") {
    get_function(uri, "ragged_run")(
        ...
        window_left,
        support_pdl(),
        /*maybe_custom_mask=*/processed_mask.has_value()
            ? to_ffi_tensor(processed_mask.value())
            : ffi::Optional<ffi::Tensor>(),
        /*maybe_mask_indptr=*/mask_indptr_opt.has_value()
            ? to_ffi_tensor(mask_indptr_opt.value())
            : ffi::Optional<ffi::Tensor>(),
