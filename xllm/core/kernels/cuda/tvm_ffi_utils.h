    ...
    switch (encode_dlpack_dtype(dlpack_dtype)) {                       \
      ...
      default:                                                         \
        TVM_FFI_ICHECK(false)                                          \
            << __PRETTY_FUNCTION__ << " failed to dispatch data type " \
...
inline void check_shape(const tvm::ffi::Tensor& a,
                        const tvm::ffi::Tensor& b,
                        const char* a_name,
                        const char* b_name) {
  TVM_FFI_ICHECK_EQ(a->ndim, b->ndim)
      << a_name << "->ndim and " << b_name << "->ndim mismatch";
  for (int i = 0; i < a->ndim; ++i) {
    TVM_FFI_ICHECK_EQ(a->shape[i], b->shape[i])
        << a_name << "->shape[" << i << "] and " << b_name << "->shape[" << i
        << "] mismatch";
  }
}
...
#define CHECK_CUDA(x)                               \
  TVM_FFI_ICHECK_EQ(x->device.device_type, kDLCUDA) \
      << #x " must be a CUDA tensor";
#define CHECK_CPU(x)                               \
  TVM_FFI_ICHECK_EQ(x->device.device_type, kDLCPU) \
      << #x " must be a host tensor";
#define CHECK_CONTIGUOUS(x) \
  TVM_FFI_ICHECK(x.IsContiguous()) << #x " must be contiguous";
...
#define CHECK_DIM(d, x) \
  TVM_FFI_ICHECK_EQ(x->ndim, d) << #x " must be a " #d "D tensor";
...
inline ffi::Tensor alloc_tensor(tvm::ffi::Shape shape,
                                DLDataType dtype,
                                DLDevice device) {
