...
void cache_select(const torch::Tensor& beam_index,
                  ...
                  int64_t beam_size,
                  int64_t layer_num) {
  check_tensor(beam_index, "beam_index", "cache_select");
  check_tensor(block_table, "block_table", "cache_select");
  check_tensor(group_offset, "group_offset", "cache_select");
  // Validate tensor lists
  for (const auto& t : x_key_block) {
    check_tensor(t, "x_key_block[i]", "cache_select");
  }
  for (const auto& t : x_value_block) {
    check_tensor(t, "x_value_block[i]", "cache_select");
  }
