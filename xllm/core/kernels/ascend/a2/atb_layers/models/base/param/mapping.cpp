...
void Mapping::ParseParam(const nlohmann::json& paramJson) {
  ...
  this->rankTableFile_ =
      FetchJsonParam<std::string>(paramJson, "rankTableFile");
  ...
  for (auto it = strategyKeyMap.begin(); it != strategyKeyMap.end(); it++) {
    ...
    parallelInfo.bufferSize =
        FetchJsonParam<uint32_t>(curParamJson, "bufferSize");
...
void Mapping::InitCommDomain(std::string defaultBackend) {
  ...
  // Create global comm
  GetSingleton<ExternalCommManager>().Init(
      this->rankTableFile_,
      static_cast<uint32_t>(this->worldSize_),
      static_cast<uint32_t>(this->rank_));

  // Create sub comm
  for (auto it = this->parallelStrategies_.begin();
       it != this->parallelStrategies_.end();
       it++) {
    std::string backend =
        it->second.backend != "" ? it->second.backend : this->defaultBackend_;

    // change to hccl if the communication channel across nodes
    int32_t currentDevice = -1;
    for (uint32_t item : it->second.rankIds) {
      if (currentDevice != -1 &&
          static_cast<int32_t>(ceil(item / this->localWorldSize_)) !=
              currentDevice) {
        backend = "hccl";
        break;
      }
    ...
    // when a rankTableFile is supplied and the communication channel spans the
    // entire world size.
    if (this->worldSize_ <= this->localWorldSize_ &&
        this->rankTableFile_ != "" &&
...
uint32_t Mapping::localWorldSize() const { return localWorldSize_; }

void Mapping::Register(ParallelType parallelType,
                       atb_speed::common::ParallelInfo parallelInfo) {
  this->parallelStrategies_[parallelType] = parallelInfo;
}

const atb_speed::common::ParallelInfo& Mapping::Get(
...
std::string Mapping::ToString() const {
  ...
  ss << "Mapping Info: worldSize: " << this->worldSize_
     ...
     << ", localWorldSize: " << this->localWorldSize_ << ",[" << WORD_EMBED_TP
     << "] wordEmbeddingTensorParallel: "
     ...
     << this->parallelStrategies_.at(LM_HEAD_DP).ToString() << ",[" << ATTN_TP
     << "] attnTensorParallel: "
     << this->parallelStrategies_.at(ATTN_TP).ToString() << ",[" << ATTN_DP
     << "] attnDataParallel: "
     << this->parallelStrategies_.at(ATTN_DP).ToString() << ",["
     << ATTN_INNER_SP << "] attnInnerSequenceParallel: "
     << this->parallelStrategies_.at(ATTN_INNER_SP).ToString() << ",[" << MLP_TP
     << "] mlpTensorParallel: "
     << this->parallelStrategies_.at(MLP_TP).ToString() << ",[" << MLP_DP
     << "] mlpDataParallel: " << this->parallelStrategies_.at(MLP_DP).ToString()
     << ",[" << MOE_TP << "] sharedMlpTensorParallel: "
     << this->parallelStrategies_.at(MOE_TP).ToString() << ",[" << MOE_EP
     << "] moeExpertParallel: "
