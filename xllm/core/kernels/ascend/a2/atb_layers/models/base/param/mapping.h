...
enum class MoeExpertParallelDegree : uint32_t {
  /// The expert parallelism where experts are deterministic on each of the
  /// devices
  STATIC = 0,
  /// The expert parallelism where experts on each device are chosen upon
  /// expected
  /// "workload" of each expert to ideally even out the amount of calculation on
  /// each device
  DYNAMIC,
  /// The mixture of static and dynamic Ep strategies, where static Ep is
  /// applied for Prefill Stage
...
enum class AttnDataParallelDegree : uint32_t {
  /// The data parallelism where weights are duplicated across devices in the
  /// same Dp communication group
  OUTER = 0,
  /// The data parallelism where weights are loaded just as if tensor
  /// parallelism were applied, yet data parallelism
...
class Mapping {
 public:
  ...
  /// Global rank
  uint32_t rank_;
  /// Path of the file contains devices' Ip and rank info to construct
  /// communication groups
  std::string rankTableFile_ = "";
  /// An indicator that shows whether commDomains are assigned to each parallel
  /// strategy
  bool isInitialized_ = false;

  /// Convert and `nlohmann::json` object to a `Mapping` object
  /// \param paramJson An `nolhmann::json` object holds all the required
  /// parameters.
  void ParseParam(const nlohmann::json& paramJson);
  /// Add a `ParallelInfo` strategy into `parallelStrategies_` with key
  /// `parallelType`
  /// \param parallelType The key of the strategy
  /// \param parallelInfo A `ParallelInfo` object that holds info of the
  /// communication group
  void Register(ParallelType parallelType,
                atb_speed::common::ParallelInfo parallelInfo);
  /// Get a `ParallelInfo` object from `parallelStrategies_` by key
  /// `parallelType`
  /// \param parallelType The key of the strategy
  /// \throw Throws out of range error if key `parallelType` is not in
  /// `parallelStrategies_`
  /// \return a `ParallelInfo` object corresponding to the parallelism strategy
  ...
  /// \return A flag indicating whether the communication domain is created
  /// successfully
