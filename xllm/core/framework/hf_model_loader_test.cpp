...
TEST(HFModelLoaderTest, Qwen35MtpModelArgsFromDenseConfig) {
  ...
  ASSERT_TRUE(loader(reader, model_args));
  EXPECT_EQ(model_args->model_type(), "qwen3_5_mtp");
  EXPECT_EQ(model_args->num_nextn_predict_layers(), 1);
  EXPECT_EQ(model_args->n_layers(), 1);
  ASSERT_EQ(model_args->layer_types().size(), 1);
  EXPECT_EQ(model_args->layer_types()[0], "full_attention");
}
...
TEST(HFModelLoaderTest, Qwen35MtpModelArgsFromMoeConfig) {
  ...
  auto model_args = std::make_shared<ModelArgs>();
  ASSERT_TRUE(loader(reader, model_args));
  EXPECT_EQ(model_args->model_type(), "qwen3_5_moe_mtp");
  EXPECT_EQ(model_args->num_nextn_predict_layers(), 2);
  EXPECT_EQ(model_args->n_layers(), 2);
  ASSERT_EQ(model_args->layer_types().size(), 2);
  EXPECT_EQ(model_args->layer_types()[0], "full_attention");
  EXPECT_EQ(model_args->layer_types()[1], "full_attention");
}
#endif
