...
std::vector<Block> MMPrefixCache::match(
  ...
  // Build the block key from PrefixCache's rolling text hash plus the hashes of
  // multimodal items overlapping this block. Without the multimodal part, two
  // requests with the same text tokens but different images or videos could
  // incorrectly reuse the same prefix-cache entry.
  for (cur_token_index = start_index; cur_token_index < n_tokens;
    ...
    if (cur_token_index == 0) {
      mm_xxh3_128bits_hash(mm_hash_values,
                           nullptr,
