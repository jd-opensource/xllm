...
TEST(BatchTest, BeamSearchExpandsFinalResultsBeyondBeamWidth) {
  torch::Device device(Device::type_torch(), 0);
  const uint32_t n_blocks = 8;
  const uint32_t block_size = 4;
  BlockManager::Options options;
  ...
  sampling_param.logprobs = true;
  sampling_param.top_logprobs = 2;
  sampling_param.beam_width = 2;
  sampling_param.num_return_sequences = 4;
  StoppingChecker stopping_checker;
  stopping_checker.set_max_generated_tokens(1);
  ...
  SequencesGroup group(
      ...
      mm_data,
      seq_params);
  auto* seq0 = group.sequences()[0].get();
  seq0->add_kv_blocks(manager.allocate(1));
  seq0->kv_state().incr_kv_cache_tokens_num(seq0->num_prompt_tokens());

  auto seq1 = std::make_unique<Sequence>(*seq0);

  std::vector<int64_t> top_tokens0 = {10, 11};
  std::vector<float> top_logprobs0 = {0.40f, 0.30f};
  Token token0(10);
  token0.logprob = 0.40f;
  token0.top_tokens = {top_tokens0.data(), top_tokens0.size()};
  token0.top_logprobs = {top_logprobs0.data(), top_logprobs0.size()};
  seq0->append_token(token0);

  std::vector<int64_t> top_tokens1 = {20, 21};
  std::vector<float> top_logprobs1 = {0.25f, 0.15f};
  Token token1(20);
  token1.logprob = 0.25f;
  token1.top_tokens = {top_tokens1.data(), top_tokens1.size()};
  token1.top_logprobs = {top_logprobs1.data(), top_logprobs1.size()};
  ...
  EXPECT_EQ(
      group.sequences()[0]->tokens()[group.sequences()[0]->num_tokens() - 1],
      10);
  EXPECT_EQ(
      group.sequences()[1]->tokens()[group.sequences()[1]->num_tokens() - 1],
      11);
  EXPECT_EQ(
      group.sequences()[2]->tokens()[group.sequences()[2]->num_tokens() - 1],
      20);
  EXPECT_EQ(
      group.sequences()[3]->tokens()[group.sequences()[3]->num_tokens() - 1],
      21);
  EXPECT_FLOAT_EQ(group.sequences()[0]->get_acc_logprob(), 0.40f);
...
TEST(BatchTest, MultiRoundBeamSearchExpandsFinalResultsBeyondBeamWidth) {
  ...
  seq_params.logprobs = true;
  seq_params.enable_schedule_overlap = false;
  seq_params.rec_type = RecType::kLlmRec;
  ...
  output.beam_sequence_group =
      torch::tensor({{{11, 21, 31}, {12, 22, 32}}},
                    torch::TensorOptions().dtype(torch::kInt32));
  output.beam_base_logprobs = torch::tensor(
      {0.50f, 0.40f}, torch::TensorOptions().dtype(torch::kFloat32));
  output.beam_source_sequence_group =
      torch::tensor({{{11, 21, 0}, {12, 22, 0}}},
                    torch::TensorOptions().dtype(torch::kInt32));
  output.sample_output.top_tokens = torch::tensor(
      {{31, 33}, {32, 34}}, torch::TensorOptions().dtype(torch::kInt64));
  output.sample_output.top_logprobs =
      torch::tensor({{0.30f, 0.20f}, {0.25f, 0.15f}},
                    torch::TensorOptions().dtype(torch::kFloat32));
  output.beam_search_output.out_logprobs = torch::tensor(
      {0.80f, 0.65f}, torch::TensorOptions().dtype(torch::kFloat32));

  batch.process_beam_sequence_group(output);

  const auto& cached_group = group.sequences()[0]->beam_seq_group_flat();
  const auto& cached_logprobs = group.sequences()[0]->beam_last_logprobs();
  ASSERT_EQ(cached_group.size(), 4);
  ASSERT_EQ(cached_logprobs.size(), 4);
  EXPECT_EQ(cached_group[0], (std::vector<int32_t>{11, 21, 31}));
  EXPECT_EQ(cached_group[1], (std::vector<int32_t>{11, 21, 33}));
  EXPECT_EQ(cached_group[2], (std::vector<int32_t>{12, 22, 32}));
  EXPECT_EQ(cached_group[3], (std::vector<int32_t>{12, 22, 34}));
  EXPECT_FLOAT_EQ(cached_logprobs[0], 0.80f);
  EXPECT_FLOAT_EQ(cached_logprobs[1], 0.70f);
  EXPECT_FLOAT_EQ(cached_logprobs[2], 0.65f);
  EXPECT_FLOAT_EQ(cached_logprobs[3], 0.55f);
