...
ForwardInput OneRecBatchInputBuilder::build_rec_forward_input(
  ...
  auto& onerec_params = input.mutable_onerec_params();
  auto& cache_data = perf_cache.cache_data;
  ...
  if (thread_pool_ && num_sequences >= kSequenceParallelGrain) {
    ...
    */
    ...
    input.meta.kv_max_seq_len = seq_len + num_decoder_embeddings;
    input.meta.q_max_seq_len = seq_len + num_decoder_embeddings;
    ...
    input.attention.device.kv_seq_lens =
        torch::empty({static_cast<int64_t>(cu_seq_lens.size())},
                     torch::TensorOptions()
                         ...
                         .device(torch::kCPU)
                         .pinned_memory(true));
    std::memcpy(input.attention.device.kv_seq_lens.data_ptr<int>(),
                cu_seq_lens.data(),
    ...
    input.attention.device.q_seq_lens =
        torch::empty({static_cast<int64_t>(q_cu_seq_lens.size())},
                     torch::TensorOptions()
                         ...
                         .device(torch::kCPU)
                         .pinned_memory(true));
    std::memcpy(input.attention.device.q_seq_lens.data_ptr<int>(),
                q_cu_seq_lens.data(),
    ...
    input.attention.device.q_cu_seq_lens =
        torch::tensor(device_q_cu_seq_lens,
                      torch::TensorOptions()
                          ...
                          .device(torch::kCPU)
                          .pinned_memory(true));
    input.attention.host.kv_seq_lens = std::move(cu_seq_lens);
    input.attention.host.q_cu_seq_lens = std::move(device_q_cu_seq_lens);
    input.attention.host.q_seq_lens = std::move(q_cu_seq_lens);
  ...
  if (thread_pool_ && num_sequences >= kSequenceParallelGrain) {
    ...
    // time-consuming)
    thread_pool_->schedule(
        [&input, num_sequences, &block_tables_promise]() mutable {
          try {
            ...
            input.attention.device.new_cache_slots =
                torch::empty({static_cast<int64_t>(paged_kv_indptr.size())},
                             torch::TensorOptions()
                                 ...
                                 .device(torch::kCPU)
                                 .pinned_memory(true));
            std::memcpy(input.attention.device.new_cache_slots.data_ptr<int>(),
  ...
  input.embedding.embedding_ids.assign(num_sequences, 0);
  input.meta.batch_id = batch_id_;
  input.embedding.request_ids.clear();
  input.embedding.request_ids.reserve(static_cast<size_t>(num_sequences));
  for (auto* group : sequence_groups_) {
    for (const auto& sequence : group->sequences()) {
      input.embedding.request_ids.emplace_back(sequence->request_id());
    }
  }
