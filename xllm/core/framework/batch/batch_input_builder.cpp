...
ForwardInput BatchInputBuilder::state_to_forward_input() {
  ...
  input.meta.batch_forward_type = state_.batch_forward_type;
  input.meta.num_sequences = static_cast<int32_t>(num_sequences_);
  input.meta.kv_max_seq_len = state_.max_seq_len;
  input.meta.q_max_seq_len = state_.q_max_seq_len;
  ...
  input.attention.host.kv_seq_lens = std::move(state_.seq_lens);
  input.attention.host.q_cu_seq_lens = std::move(q_cu_seq_lens);
  input.attention.host.q_seq_lens = std::move(state_.q_seq_lens);
  ...
  for (auto& mgr_tables : state_.multi_block_tables) {
    ...
    input.multi_block_tables.push_back(
  ...
  input.embedding.embedding_ids = std::move(state_.embedding_ids);
  input.embedding.linear_state_ids = std::move(state_.linear_state_ids);
  ...
  input.embedding.request_ids = std::move(state_.request_ids);
  input.embedding.extra_token_ids = std::move(state_.extra_token_ids);
  if (!state_.mtp_shifted_token_ids.empty()) {
    ...
    input.embedding.mtp_shifted_token_ids = mtp_tensor;
    input.mtp_shifted_token_ids = mtp_tensor;
  }
  if (!state_.mtp_bootstrap_embeddings.empty()) {
    CHECK_EQ(state_.mtp_bootstrap_row_idxes.size(),
             state_.mtp_bootstrap_embeddings.size());
    input.embedding.mtp_bootstrap_row_idxes =
        std::move(state_.mtp_bootstrap_row_idxes);
    input.embedding.mtp_bootstrap_embeddings =
...
void BatchInputBuilder::process_swap_block_infos(ForwardInput& forward_input) {
  ...
  if (::xllm::BeamSearchConfig::get_instance().enable_block_copy_kernel()) {
    ...
    if (!kernel_input.has_overlap) {
      ...
      input.block_copy.cum_sum = build_pinned_int_tensor(kernel_input.cum_sum);
    }
#else
    const BlockCopyKernelInputData kernel_input =
        build_block_copy_kernel_input_data(swap_blocks,
