...
void LlmDataDistTransfer::allocate_kv_cache(
  ...
  for (int64_t i = 0; i < num_layers; ++i) {
    ...
    if (enable_lighting_indexer_) {
      ...
      acl_ret = aclrtMalloc(
          &index_cache_buffer, index_cache_size_per_layer, ACL_MEM_MALLOC_HUGE_ONLY);
      CHECK(acl_ret == ACL_SUCCESS) << "aclrtMalloc index cache failed.";
      index_cache_addrs.emplace_back(reinterpret_cast<uint64_t>(index_cache_buffer));
...
bool LlmDataDistTransfer::push_kv_blocks(
  ...
  for (int64_t layer_index = 0; layer_index < num_layers_; ++layer_index) {
    ...
    for (const auto& pair : merged_kv_infos) {
      ...
      CacheIndex index_cache_index{kv_info.dst_cluster_id, index_cache_.cache_id};
