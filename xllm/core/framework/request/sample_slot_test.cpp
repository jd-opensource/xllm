...
class CharTokenizer final : public Tokenizer {
 public:
  bool encode(const std::string_view& text,
    ...
    size_t pos = 0;
    while (pos < text.size()) {
      if (text.size() - pos >= kEmbToken.size() &&
          std::string_view(text.data() + pos, kEmbToken.size()) == kEmbToken) {
 ...
 private:
  ...
  static constexpr std::string_view kEmbToken = "<emb_0>";
};
