...
class CompositeBlockManager : public BlockManager {
 public:
  ...
  // Leaf serving `type`, or nullptr if none. Public so the pool can route
  // typed APIs (linear-state restore/save, beam KV copy-on-write) to a
  // specific leaf without dynamic_cast through every leaf.
