...
void HierarchyBlockManagerPool::offload_via_host(Sequence* sequence,
  ...
  size_t cached_block_num = sequence->host_kv_state().shared_kv_blocks_num();

  uint32_t needed_offload_num =
      token_ids.size() / options_.block_size() - cached_block_num;

  if (needed_offload_num < sequence->get_offload_batch() && !finish) {
    return;
  }

  uint32_t needed_block_num =
      token_ids.size() / options_.block_size() - host_blocks->size();
...
void HierarchyBlockManagerPool::load_direct(Sequence* sequence,
  ...
  uint32_t store_shared_num =
      PrefixCache::match_in_kvcache_store(options_.tp_size(), keys);
