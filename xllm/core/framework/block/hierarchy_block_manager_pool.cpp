...
void HierarchyBlockManagerPool::allocate_host_shared(Sequence* sequence) {
  if (sequence->host_kv_state().num_kv_blocks() == 0 &&
    ...
    int64_t int_rate_percent = static_cast<int64_t>(
        shared_blocks.size() * 100.0 /
        ((sequence->tokens().size() + options_.block_size() - 1) /
         options_.block_size()));
    HISTOGRAM_OBSERVE(host_prefix_cache_block_matched_rate, int_rate_percent);
    HISTOGRAM_OBSERVE(host_prefix_cache_block_matched_num,
                      shared_blocks.size());
...
bool HierarchyBlockManagerPool::update_prefetch_result(
  ...
  for (auto& prefill_sequence : request->sequences()) {
    ...
    if (prefetch_result && success_cnt > 0) {
      ...
      int64_t int_rate_percent =
          static_cast<int64_t>(success_cnt * 100.0 / host_blocks.size());
      HISTOGRAM_OBSERVE(store_prefix_cache_block_matched_rate,
                        int_rate_percent);
      HISTOGRAM_OBSERVE(store_prefix_cache_block_matched_num, success_cnt);
    }
  }
...
void HierarchyBlockManagerPool::transfer_blocks(std::vector<Batch>& batches) {
  ...
  size_t load_block_num = 0;
  for (size_t i = 0; i < batches.size(); i++) {
    if (!load_block_transfer_infos_[i].empty()) {
      load_block_num += load_block_transfer_infos_[i].size();
  ...
  if (load_block_num > 0) {
    HISTOGRAM_OBSERVE(load_kvcache_block_num, load_block_num);
  }
...
void HierarchyBlockManagerPool::transfer_blocks() {
  ...
  if (offload_block_num > 0) {
    HISTOGRAM_OBSERVE(offload_kvcache_block_num, offload_block_num);
  }
}
