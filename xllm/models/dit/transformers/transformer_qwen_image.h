...
// // TODO: a factory function that provides activation functions based on
// string
// // input
...
//           "norm_q",
//           RMSNorm(dim_head, eps, /*elementwise_affine=*/true,
//           /*bias=*/false));
...
//       norm_cross_{nullptr};
//   xllm::dit::DiTParallelLinear to_q_{nullptr}, to_k_{nullptr},
//   to_v_{nullptr}; xllm::dit::DiTParallelLinear add_k_proj_{nullptr},
//   add_v_proj_{nullptr},
//       add_q_proj_{nullptr};
...
//                         torch::nn::Functional(
//                             std::function<torch::Tensor(const
//                             torch::Tensor&)>(
...
//         "linear2",
//         layer::AddMatmulWeightTransposed(inner_dim, dim_out, true,
//         options_));
//   }
...
//     if (index.defined()) {
//       // Assuming mod_params batch dim is 2*actual_batch (chunked into 2
//       parts)
...
//       gate_result = torch::where(index_expanded == 0, gate_0_exp,
//       gate_1_exp);
//     } else {
...
// std::tuple<int64_t, std::optional<torch::Tensor>,
// std::optional<torch::Tensor>> compute_text_seq_len_from_mask(
...
//   // Compute per-sample length: max position + 1 if active, else use full
//   length auto max_positions = std::get<0>(active_positions.max(/*dim=*/1));
...
//       device); vid_freqs.emplace_back(video_freq);
...
//                                             int64_t idx,
//                                             torch::Device device) {
//     std::string key = std::to_string(idx) + "_" + std::to_string(height) +
//     "_" +
//                       std::to_string(width);
...
//     if (scale_rope_) {
//       auto neg_part = freqs_neg[2].slice(0, -(width / 2),
//       freqs_neg[2].size(0)); auto pos_part = freqs_pos[2].slice(0, 0, width /
//       2); freqs_width = torch::cat({neg_part, pos_part}, 0)
//                         .view({1, 1, width, -1})
//                         .expand({frame, height, width, -1});
//     } else {
...
//       if (idx != layer_num) {
//         video_freq =
//             _compute_video_freqs_with_cache(frame, height, width, idx,
//             device);
//       } else {
//         video_freq =
//             _compute_condition_freqs_with_cache(frame, height, width,
//             device);
//       }
//       vid_freqs_list.push_back(video_freq);
...
//     // Image normalization
//     img_norm1_ = register_module("img_norm1", AdaLayerNorm(context, dim,
//     eps));
//     // Attention module
//     auto attn_ =
...
//     img_norm2_ = register_module("img_norm2", AdaLayerNorm(context, dim,
//     eps));

//     // Image MLP
//     img_mlp_ =
//         register_module("img_mlp", qwenimage::FeedForward(context, dim,
//         dim));
...
//     txt_norm1_ = register_module("txt_norm1", AdaLayerNorm(context, dim,
//     eps));

//     // Text normalization 2
//     txt_norm2_ = register_module("txt_norm2", AdaLayerNorm(context, dim,
//     eps));

//     // Text MLP
//     txt_mlp_ =
//         register_module("txt_mlp", qwenimage::FeedForward(context, dim,
//         dim));
//   }
...
//           "pos_embed",
//           QwenEmbedLayer3DRope(context, /*theta=*/10000, axes_dims_rope,
//           true));
//     } else {
...
//         "time_text_embed",
//         QwenTimestepProjEmbeddings(context, inner_dim,
//         use_additional_t_cond));

//     // Text normalization
//     txt_norm_ = register_module(
//         "txt_norm", qwenimage::RMSNorm(joint_attention_dim, 1e-6, true,
//         false));
...
//         "proj_out",
//         layer::AddMatmulWeightTransposed(
//             inner_dim, patch_size * patch_size * out_channels, true,
//             options_));
...
//     new_encoder_hidden_states =
//     txt_norm_->forward(new_encoder_hidden_states); new_encoder_hidden_states
//     = txt_in_->forward(new_encoder_hidden_states);
...
//     if (!use_step_cache) {
//       for (int64_t index_block = 0; index_block <
