...
//     return std::vector<torch::Tensor>(split_result.begin(),
//     split_result.end());
//   }
...
//     prompt_embeds =
//         prompt_embeds.view({batch_size * num_images_per_prompt, seq_len,
//         -1});
...
//     latents =
//         latents.view({batch_size, height / 2, width / 2, channels / 4, 2,
//         2});
...
//                                   int64_t seed,
//                                   torch::Device device) {
//     auto image_latents = _retrieve_latents(vae_->encode(image), seed,
//     "argmax"); auto latents_mean =
...
//       }
//       if (img.size(0) > 1) {
//         LOG(ERROR) << "currently QwenImageEdit doesn't support batch
//         inference"
...
//         aspect_ratio =
//             static_cast<double>(image_list[i].size(2)) /
//             image_list[i].size(1);
//         auto [condition_width, condition_height] =
//             xllm::dit::calculate_dimensions(CONDITION_IMAGE_SIZE,
//             aspect_ratio);
...
//     if (do_true_cfg) {
//       // inplace update negative_prompt_embeds and
//       negative_prompt_embeds_mask _encode_prompt(condition_images,
//                      negative_prompts,
//                      negative_prompt_embeds,
...
//         auto comb_pred =
//             noise_preds[1] + true_cfg_scale * (noise_preds[0] -
//             noise_preds[1]);
