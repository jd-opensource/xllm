 ...
 public:
  ...
  DiTForwardOutput forward(const DiTForwardInput& forward_input) {
    ...
    auto image = forward_input.images.defined()
                     ...
                     : std::nullopt;
    auto mask_image = forward_input.mask_images.defined()
                          ? std::make_optional(forward_input.mask_images)
                          : std::nullopt;
    auto masked_image_latents =
        forward_input.masked_image_latents.defined()
            ? std::make_optional(forward_input.masked_image_latents)
            : std::nullopt;
