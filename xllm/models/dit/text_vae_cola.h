...
class TextVAEBlockImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward(const torch::Tensor& x,
    ...
    LOG(INFO) << "VAE-blk: input: mean=" << x.mean().item<float>()
              << ", std=" << x.std().item<float>();
    ...
    // QK normalization on full dim (before reshape to heads).
    auto dtype = q.dtype();
    ...
    torch::Tensor attn_out;
    {
      ...
      LOG(INFO) << "VAE-blk: attn_weight: shape=[" << attn_weight.size(0) << ","
                << attn_weight.size(1) << "," << attn_weight.size(2) << ","
                << attn_weight.size(3)
                << "], mean=" << attn_weight.mean().item<float>()
                << ", max=" << attn_weight.max().item<float>();
    ...
    LOG(INFO) << "VAE-blk: attn_result: mean="
              << attn_result.mean().item<float>()
              << ", std=" << attn_result.std().item<float>();

    // --- Post-norm residual (post_norm=True) ---------------------------------
    // Attention sublayer: x_new = norm_attn(x) + attn
    auto q_part = norm_attn_(x) + attn_result;  // (L_q, dim)
    LOG(INFO) << "VAE-blk: after norm_attn: mean="
              << q_part.mean().item<float>()
              << ", std=" << q_part.std().item<float>();

    // FFN sublayer: x_new = q_part + norm_ffn(ffn(q_part))
    auto residual = q_part;
    auto ffn_out = ffn_out_(ffn_act_(ffn_proj_(q_part)));
    q_part = residual + norm_ffn_(ffn_out);
    LOG(INFO) << "VAE-blk: after norm_ffn: mean=" << q_part.mean().item<float>()
              << ", std=" << q_part.std().item<float>();
...
class ColaTextVAEDecoderImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor decode(const torch::Tensor& z,
    ...
    auto hidden = in_layer_(z);  // (L_q, dim)
    LOG(INFO) << "VAE-dec: input dtype=" << torch::toString(z.scalar_type())
              << ", hidden dtype=" << torch::toString(hidden.scalar_type())
              << ", autocast=" << at::autocast::is_autocast_enabled(at::kCUDA);
    {
      auto s = hidden.cpu();
    ...
    hidden = final_layer_(hidden);  // (L_q*ps, vocab)
    {
      auto s = hidden.cpu();
