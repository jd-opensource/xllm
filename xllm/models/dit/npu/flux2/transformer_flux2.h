...
using dit::LinearType;
using dit::SequenceParallelPadManager;
using dit::SpOptions;
using dit::TpOptions;
...
class Flux2FeedForwardImpl : public torch::nn::Module {
 public:
  explicit Flux2FeedForwardImpl(const ModelContext& context,
    ...
    auto linear_in = DiTParallelLinear(inner_dim,
                                       inner_dim * 6,
                                       false,
                                       options_,
                                       linear_type,
  ...
  void load_state_dict(const StateDict& state_dict) {
    int64_t tp_size = FLAGS_tp_size > 1 ? FLAGS_tp_size : 1;
    if (tp_size > 1) {
...
class Flux2AttentionImpl : public torch::nn::Module {
 public:
  explicit Flux2AttentionImpl(const ModelContext& context,
    ...
    if (FLAGS_sp_size > 1 && FLAGS_tp_size > 1) {
      linear_type = LinearType::TensorAndSequenceParallel;
      sp_option = SpOptions(/*head_num=*/heads_,
                            /*head_dim=*/head_dim_,
                            /*hidden_size=*/query_dim_,
      ...
      tp_options = TpOptions(
          ...
          /*need_scatter=*/false,
          /*process_group=*/parallel_args_.dit_tp_group_);
    } else if (FLAGS_sp_size > 1) {
      linear_type = LinearType::SequenceParallel;
      sp_option = SpOptions(/*head_num=*/heads_,
                            /*head_dim=*/head_dim_,
                            /*hidden_size=*/query_dim_,
                            /*before_attention=*/true,
                            /*process_group=*/parallel_args_.dit_sp_group_);
    } else if (FLAGS_tp_size > 1) {
      linear_type = LinearType::TensorParallel;
      tp_options = TpOptions(
          /*column_parallel=*/true,
          /*tp_rank=*/parallel_args_.dit_tp_group_->rank(),
    ...
    auto to_q = DiTParallelLinear(query_dim_,
                                  out_dim_,
                                  false,
                                  options_,
                                  linear_type,
    ...
    auto to_k = DiTParallelLinear(query_dim_,
                                  out_dim_,
                                  false,
                                  options_,
                                  linear_type,
    ...
    auto to_v = DiTParallelLinear(query_dim_,
                                  out_dim_,
                                  false,
                                  options_,
                                  linear_type,
    ...
    // ── Output projections (before_attention=false for SP) ──
    LinearType to_out_type = LinearType::Default;
    SpOptions out_sp_option;
    std::optional<TpOptions> tp_out_options = std::nullopt;

    if (FLAGS_sp_size > 1 && FLAGS_tp_size > 1) {
      to_out_type = LinearType::TensorAndSequenceParallel;
      out_sp_option = SpOptions(/*head_num=*/heads_,
                                /*head_dim=*/head_dim_,
                                /*hidden_size=*/query_dim_,
      ...
      to_out_type = LinearType::SequenceParallel;
      out_sp_option = SpOptions(/*head_num=*/heads_,
                                /*head_dim=*/head_dim_,
                                /*hidden_size=*/query_dim_,
                                /*before_attention=*/false,
    ...
    auto to_out = DiTParallelLinear(out_dim_,
                                    query_dim_,
                                    false,
                                    options_,
                                    to_out_type,
    ...
    if (added_kv_proj_dim_ > 0) {
      ...
      SpOptions add_out_sp_option;
      std::optional<TpOptions> tp_add_out_options = std::nullopt;

      if (FLAGS_sp_size > 1 && FLAGS_tp_size > 1) {
        to_add_out_type = LinearType::TensorAndSequenceParallel;
        add_out_sp_option =
            SpOptions(/*head_num=*/heads_,
                      /*head_dim=*/head_dim_,
                      /*hidden_size=*/query_dim_,
                      /*before_attention=*/false,
                      /*process_group=*/parallel_args_.dit_sp_group_);
        ...
        to_add_out_type = LinearType::SequenceParallel;
        add_out_sp_option =
            SpOptions(/*head_num=*/heads_,
                      /*head_dim=*/head_dim_,
                      /*hidden_size=*/query_dim_,
  ...
  std::tuple<torch::Tensor, torch::Tensor> forward(
    ...
    // Unpad before RoPE: remove padding added for SP divisibility
    if (FLAGS_sp_size > 1) {
      ...
      SequenceParallelPadManager::getInstance().unpad_tensor(
          query, /*tensor_name=*/"hidden_states", /*dim=*/1);
      SequenceParallelPadManager::getInstance().unpad_tensor(
          key, /*tensor_name=*/"hidden_states", /*dim=*/1);
      SequenceParallelPadManager::getInstance().unpad_tensor(
          value, /*tensor_name=*/"hidden_states", /*dim=*/1);
    }
    ...
    // Pad back after attention for output projections
    if (FLAGS_sp_size > 1) {
      encoder_output = SequenceParallelPadManager::getInstance().pad_tensor(
          encoder_output, /*tensor_name=*/"encoder_hidden_states", /*dim=*/1);
      hidden_output = SequenceParallelPadManager::getInstance().pad_tensor(
          hidden_output, /*tensor_name=*/"hidden_states", /*dim=*/1);
    }
...
class Flux2TransformerBlockImpl : public torch::nn::Module {
 public:
  ...
  std::tuple<torch::Tensor, torch::Tensor> forward(
      ...
      const torch::Tensor& image_rotary_emb) {
    int sp_rank =
        (FLAGS_sp_size > 1) ? parallel_args_.dit_sp_group_->rank() : 0;
...
class Flux2ParallelSelfAttentionImpl : public torch::nn::Module {
 public:
  explicit Flux2ParallelSelfAttentionImpl(const ModelContext& context,
    ...
    LinearType qkv_linear_type = LinearType::Default;
    SpOptions sp_option;
    std::optional<TpOptions> tp_options = std::nullopt;

    if (FLAGS_sp_size > 1 && FLAGS_tp_size > 1) {
      qkv_linear_type = LinearType::TensorAndSequenceParallel;
      sp_option = SpOptions(/*head_num=*/heads_,
                            /*head_dim=*/head_dim_,
                            /*hidden_size=*/query_dim_,
                            /*before_attention=*/true,
      ...
      qkv_linear_type = LinearType::SequenceParallel;
      sp_option = SpOptions(/*head_num=*/heads_,
                            /*head_dim=*/head_dim_,
                            /*hidden_size=*/query_dim_,
                            /*before_attention=*/true,
      ...
      qkv_linear_type = LinearType::TensorParallel;
      tp_options = TpOptions(
          /*column_parallel=*/true,
          /*tp_rank=*/parallel_args_.dit_tp_group_->rank(),
          /*tp_size=*/FLAGS_tp_size,
    ...
    LinearType mlp_linear_type = LinearType::Default;
    std::optional<TpOptions> mlp_tp_options = std::nullopt;

    if (FLAGS_tp_size > 1) {
      mlp_linear_type = LinearType::TensorParallel;
    ...
    auto to_out = DiTParallelLinear(out_input_dim,
                                    out_dim_,
                                    false,
                                    options_,
                                    linear_out_type,
  ...
  torch::Tensor forward(const torch::Tensor& hidden_states,
    ...
    // ── SP: Pad back after attention
    if (FLAGS_sp_size > 1) {
      attn_output = SequenceParallelPadManager::getInstance().pad_tensor(
          attn_output, "hidden_states", 1);
    }
    // ── SP: all2all to gather heads back for attn_output ──
    if (FLAGS_sp_size > 1) {
      int64_t sp_size = FLAGS_sp_size;
      attn_output = attn_output.view({batch_size, -1, local_heads, head_dim});

      auto fn = parallel_state::all_to_all_4D(attn_output,
                                              /*scatter_dim=*/1,
                                              /*gather_dim=*/2,
                                              /*async=*/false,
      ...
      // In combined TP+SP mode, SP gathers heads within the TP partition,
      // so the output hidden size is query_dim_/tp_size.
      int64_t out_hidden =
          (FLAGS_tp_size > 1) ? query_dim_ / FLAGS_tp_size : query_dim_;
      attn_output = all2all_out.contiguous().view({batch_size, -1, out_hidden});
    }
...
class Flux2Transformer2DModelImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward(const torch::Tensor& hidden_states_input,
    ...
    // SP: Pad sequence dims to be divisible by sp_size
    if (FLAGS_sp_size > 1) {
      encoder_hidden_states =
          SequenceParallelPadManager::getInstance().pad_tensor(
              ...
              /*tensor_name=*/"encoder_hidden_states",
              /*dim=*/1);
      hidden_states = SequenceParallelPadManager::getInstance().pad_tensor(
          hidden_states,
    ...
    // SP: Split sequence along dim=1 across SP ranks
    if (FLAGS_sp_size > 1) {
      hidden_states = split_sequence(hidden_states,
                                     /*dim=*/1,
                                     parallel_args_.dit_sp_group_);
      encoder_hidden_states = split_sequence(encoder_hidden_states,
                                             /*dim=*/1,
                                             parallel_args_.dit_sp_group_);
    }
    ...
    int64_t txt_len = 0;  // real text sequence length (without SP padding)
    if (FLAGS_sp_size > 1) {
      ...
      SequenceParallelPadManager::getInstance().unpad_tensor(
          hidden_states, /*tensor_name=*/"hidden_states", /*dim=*/1);
      SequenceParallelPadManager::getInstance().unpad_tensor(
          encoder_hidden_states,
          /*tensor_name=*/"encoder_hidden_states",
          /*dim=*/1);
    ...
    txt_len = encoder_hidden_states.size(1);
    // ── Merge into single stream: [txt_seq, img_seq]
    hidden_states = torch::cat({encoder_hidden_states, hidden_states}, 1);
    // ── Re-split merged sequence for single-stream SP
    if (FLAGS_sp_size > 1) {
      hidden_states = SequenceParallelPadManager::getInstance().pad_tensor(
          hidden_states, /*tensor_name=*/"hidden_states", /*dim=*/1);
      hidden_states = split_sequence(
          hidden_states, /*dim=*/1, parallel_args_.dit_sp_group_);
    }
    ...
    // ── Extract image hidden states from the merged sequence
    torch::Tensor output_hidden;
    if (FLAGS_sp_size > 1) {
      // Gather and unpad the merged sequence (was re-split for single-stream
      // SP)
      hidden_states = gather_sequence(
          hidden_states, /*dim=*/1, parallel_args_.dit_sp_group_);
      int64_t merged_pad =
          SequenceParallelPadManager::getInstance().get("hidden_states");

      if (merged_pad > 0) {
        hidden_states =
            hidden_states.narrow(1, 0, hidden_states.size(1) - merged_pad);
      }
      output_hidden =
          hidden_states.narrow(1, txt_len, hidden_states.size(1) - txt_len);

    } else {
      int64_t encoder_seq_len = encoder_hidden_states.size(1);
      int64_t length = hidden_states.size(1) - encoder_seq_len;
      output_hidden = hidden_states.narrow(
          1, encoder_seq_len, std::max(length, int64_t(0)));
    }
