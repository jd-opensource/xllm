...
class WanCausalConv3DImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward(
    ...
    // Trim halo columns after conv — exchange() adds W columns for
    // neighbor context, conv with kernel=3, pad=1 produces output
    // that is 2 wider in W, trim 1 from each side.
    if (use_halo) {
  ...
  torch::Tensor forward(
    ...
    if (mode_ == "upsample3d" && feat_cache) {
      ...
      if ((*feat_cache)[idx].numel() == 0) {
        ...
        if (torch::equal(rep_tensor_, feat_cache->at(idx))) {
          x = time_conv_->forward(x, torch::nullopt, ctx);
        } else {
          x = time_conv_->forward(x, (*feat_cache)[idx], ctx);
        }
    ...
    if (ctx && ctx->is_parallel() && is_upsample) {
      ...
      if (ctx->has_left() && ctx->has_right()) {
        x = x.slice(/*dim=*/-1, 2, cur_w - 2);
      } else if (!ctx->has_left()) {
        x = x.slice(/*dim=*/-1, 0, cur_w - 2);
      } else {
        x = x.slice(/*dim=*/-1, 2, cur_w);
      }
    }
...
class WanResidualBlockImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward(
    ...
    torch::Tensor h;
    if (in_dim_ != out_dim_) {
      h = conv_shortcut_->forward(x, torch::nullopt, ctx);
    } else {
      h = x;
    }
    ...
    if (feat_cache) {
      ...
      x = conv1_->forward(x, (*feat_cache)[idx], ctx);
      (*feat_cache)[idx] = cache_x;
      (*feat_idx)[0] += 1;
    } else {
      x = conv1_->forward(x, torch::nullopt, ctx);
    }
    ...
    if (feat_cache) {
      ...
      x = conv2_->forward(x, (*feat_cache)[idx], ctx);
      (*feat_cache)[idx] = cache_x;
      (*feat_idx)[0] += 1;
    } else {
      x = conv2_->forward(x, torch::nullopt, ctx);
    }
...
class WanAttentionBlockImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward(torch::Tensor x, VaeSpatialParallel* ctx = nullptr) {
 ...
 public:
  ...
  torch::Tensor forward(
    ...
    if (feat_cache) {
      ...
      x = conv_in_->forward(x, (*feat_cache)[idx], ctx);
      (*feat_cache)[idx] = cache_x;
      (*feat_idx)[0] += 1;
    } else {
      x = conv_in_->forward(x, torch::nullopt, ctx);
    }
    ...
    if (feat_cache) {
      ...
      x = conv_out_->forward(x, (*feat_cache)[idx], ctx);
      (*feat_cache)[idx] = cache_x;
      (*feat_idx)[0] += 1;
    } else {
      x = conv_out_->forward(x, torch::nullopt, ctx);
    }
    return x;
  }
...
class AutoencoderKLWanImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor encode_(const torch::Tensor& videos) {
    ...
    for (int64_t i = 0; i < iter_; ++i) {
      ...
      if (i == 0) {
        auto x_slice = x.index({torch::indexing::Slice(),
                                ...
                                torch::indexing::Slice(),
                                torch::indexing::Slice()});
        enc_outputs.push_back(encoder_(
            x_slice, enc_feat_map_, enc_conv_idx_, parallel_ctx_.get()));
      } else {
    ...
    out = quant_conv_->forward(out, torch::nullopt, parallel_ctx_.get());
  ...
  DecoderOutput decode_(const torch::Tensor& latents) {
    ...
    clear_cache();
    processed_latents = post_quant_conv_->forward(
        processed_latents, torch::nullopt, parallel_ctx_.get());
