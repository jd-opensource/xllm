...
class QwenImageBaseModule : public torch::nn::Module {
 public:
  virtual torch::Tensor forward(
      ...
      VaeSpatialParallel* ctx = nullptr) = 0;
...
class QwenImageCausalConv3dImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward(const torch::Tensor& x,
                        ...
                        VaeSpatialParallel* ctx = nullptr) {
    auto padding_vec = padding_;
    auto result_x = x;

    bool use_halo =
        (ctx && ctx->is_parallel() && padding_[1] == 1 && padding_[2] == 1);
...
class QwenImageResampleImpl : public QwenImageBaseModule {
 public:
  ...
  torch::Tensor forward(
      ...
      std::shared_ptr<std::vector<torch::Tensor>> feat_cache = nullptr,
      std::shared_ptr<std::vector<int64_t>> feat_idx = nullptr,
      VaeSpatialParallel* ctx = nullptr) override {
    if (feat_idx == nullptr) {
      feat_idx =
    ...
    if (mode_ == "upsample3d" && feat_cache && feat_idx) {
      ...
      if (idx < feat_cache->size() && feat_cache->at(idx).defined()) {
        ...
        if (torch::equal(rep_tensor_, feat_cache->at(idx))) {
          result_x = time_conv_->forward(result_x, torch::Tensor(), ctx);
        } else {
          result_x = time_conv_->forward(result_x, feat_cache->at(idx), ctx);
        }
    ...
    if (mode_ == "downsample3d" && feat_cache && feat_idx) {
      ...
      if (idx < feat_cache->size() && feat_cache->at(idx).defined()) {
        ...
        result_x = time_conv_->forward(concat_x, torch::Tensor(), ctx);
        feat_cache->at(idx) = cache_x;
        (*feat_idx)[0]++;
      } else {
...
class QwenImageResidualBlockImpl : public QwenImageBaseModule {
 public:
  ...
  torch::Tensor forward(
    ...
    if (conv_shortcut_) {
      h = conv_shortcut_->forward(x, torch::Tensor(), ctx);
    } else {
    ...
    if (feat_cache && feat_idx) {
      ...
      result_x = conv1_->forward(result_x, feat_cache->at(idx), ctx);
      feat_cache->at(idx) = cache_x;
      (*feat_idx)[0]++;
    } else {
      result_x = conv1_->forward(result_x, torch::Tensor(), ctx);
    }
    ...
    if (feat_cache && feat_idx) {
      ...
      result_x = conv2_->forward(result_x, feat_cache->at(idx), ctx);
      feat_cache->at(idx) = cache_x;
      (*feat_idx)[0]++;
    } else {
      result_x = conv2_->forward(result_x, torch::Tensor(), ctx);
    }
 ...
 public:
  ...
  torch::Tensor forward(
    ...
    if (feat_cache && feat_idx) {
      ...
      result_x = conv_in_->forward(x, feat_cache->at(idx), ctx);
      feat_cache->at(idx) = cache_x;
      (*feat_idx)[0]++;
    } else {
      result_x = conv_in_->forward(x, torch::Tensor(), ctx);
    }
    ...
    if (feat_cache && feat_idx) {
      ...
      result_x = conv_out_->forward(result_x, feat_cache->at(idx), ctx);
      feat_cache->at(idx) = cache_x;
      (*feat_idx)[0]++;
    } else {
      result_x = conv_out_->forward(result_x, torch::Tensor(), ctx);
    }
 ...
 public:
  ...
  torch::Tensor forward(
    ...
    if (feat_cache) {
      ...
      result_x = conv_in_->forward(result_x, feat_cache->at(idx), ctx);
      feat_cache->at(idx) = cache_x;
      (*feat_idx)[0]++;
    } else {
      result_x = conv_in_->forward(result_x, torch::Tensor(), ctx);
    }
...
class AutoencoderKLQwenImageImpl : public torch::nn::Module {
 public:
  ...
  void set_parallel_ctx(std::unique_ptr<VaeSpatialParallel> ctx) {
    parallel_ctx_ = std::move(ctx);
  }
  VaeSpatialParallel* parallel_ctx() const { return parallel_ctx_.get(); }
  ...
  torch::Tensor _encode(const torch::Tensor& x_input) {
    ...
    if (parallel_ctx_ && parallel_ctx_->is_parallel()) {
    ...
    for (int64_t i = 0; i < iter; i++) {
      ...
      enc_outputs.push_back(encoder_->forward(
          tile, enc_feat_map_, enc_conv_idx_, parallel_ctx_.get()));
    }
    torch::Tensor out = torch::cat(enc_outputs, 2);

    auto enc = quant_conv_->forward(out, torch::Tensor(), parallel_ctx_.get());
  ...
  DecoderOutput _decode(const torch::Tensor& z_input, bool return_dict = true) {
    ...
    clear_cache();
    auto x = post_quant_conv_->forward(z, torch::Tensor(), parallel_ctx_.get());
    ...
    for (int64_t i = 0; i < num_frame; i++) {
      ...
      dec_outputs.push_back(
          decoder_->forward(frame, feat_map_, conv_idx_, parallel_ctx_.get()));
    }
 ...
 private:
  ...
  std::unique_ptr<VaeSpatialParallel> parallel_ctx_;
