 ...
 public:
  ...
  void load_model(std::unique_ptr<DiTModelLoader> loader) {
    ...
    if (text_encoder_loader) {
      try {
        // Use load_model to load weights; handles visual and language_model
        // automatically. DiTFolderLoader inherits from ModelLoader for
        // conversion.
  ...
  torch::Tensor encode_prompt_qwen(std::vector<std::string>& prompt,
    ...
    // LongCat-specific method. Using 2D positions [3, seq_len] to enable
    // MROPE which matches HuggingFace's Qwen2_5_VL behavior exactly
