    ...
    if (!forward_input.meta.batch_forward_type.is_decode()) {
      max_seq_len_ =
          ::xllm::SchedulerConfig::get_instance().enable_chunked_prefill()
              ? std::max(forward_input.meta.kv_max_seq_len, max_seq_len_)
              : 128;
      if (::xllm::SchedulerConfig::get_instance().enable_chunked_prefill()) {
        attn_mask = attn_mask_.get_attn_mask(
            max_seq_len_, cos_pos.dtype().toScalarType(), cos_pos.device());

        int batch_size = forward_input.attention.host.q_seq_lens.size();
        if (batch_size > 0) {
          std::vector<torch::Tensor> req_mask_vec;
          req_mask_vec.reserve(batch_size);

          for (int j = 0; j < batch_size; j++) {
            int start = forward_input.attention.host.kv_seq_lens[j] -
                        forward_input.attention.host.q_seq_lens[j];
            int end = forward_input.attention.host.kv_seq_lens[j];
