 ...
 public:
  ...
  torch::Tensor forward(torch::Tensor& x,
    ...
    return decoder_layer_(
        x, cos_pos, sin_pos, attn_mask, kv_cache, input, node_id);
  }
...
class LlamaModelImpl : public torch::nn::Module {
 public:
  ...
  ModelOutput forward(const ForwardInput& input,
    ...
    ForwardInput layer_input = input;
    ...
    torch::Tensor max_of_seq = torch::max(input.attention.device.kv_seq_lens);
    max_seq_len_ =
    ...
    if (::xllm::SchedulerConfig::get_instance().enable_chunked_prefill()) {
      const int32_t batch_size =
          static_cast<int32_t>(input.attention.host.q_seq_lens.size());
      std::vector<torch::Tensor> req_mask_vec;
      req_mask_vec.reserve(batch_size);

      for (int32_t i = 0; i < batch_size; i++) {
        const int32_t start = input.attention.host.kv_seq_lens[i] -
                              input.attention.host.q_seq_lens[i];
        const int32_t end = input.attention.host.kv_seq_lens[i];
