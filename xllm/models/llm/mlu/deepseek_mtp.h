...
class DeepseekMultiTokenPredictorLayerImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward(torch::Tensor embed,
    ...
    const auto& embedding_data =
        input_params.mm_data.get<torch::Tensor>("embedding");
    CHECK(embedding_data.has_value())
        << "embedding is not defined or has no value in input_params.mm_data";
    torch::Tensor previous_hidden_states = embedding_data.value();
