...
#include "core/layers/qwen35_decoder_layer.h"
#include "llm_model_base.h"

namespace xllm {

class QWen35ModelImpl : public LlmModelImplBase<layer::Qwen35DecoderLayer> {
 public:
  QWen35ModelImpl(const ModelContext& context)
      : LlmModelImplBase<layer::Qwen35DecoderLayer>("qwen35",
    ...
    for (int32_t i = 0; i < model_args.n_layers(); i++) {
      auto layer = layer::Qwen35DecoderLayer(context, i);
...
TORCH_MODULE(QWen35Model);

class QWen35ForCausalLMImpl : public LlmForCausalLMImplBase<QWen35Model> {
 public:
  QWen35ForCausalLMImpl(const ModelContext& context)
      : LlmForCausalLMImplBase<QWen35Model>(context) {}
...
TORCH_MODULE(QWen35ForCausalLM);

REGISTER_CAUSAL_MODEL(qwen35, QWen35ForCausalLM);

REGISTER_MODEL_ARGS(qwen35, [&] {
  LOAD_ARG_OR(model_type, "model_type", "qwen35");
