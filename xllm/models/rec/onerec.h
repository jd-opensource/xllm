...
class OneRecModelImpl : public torch::nn::Module {
 public:
  ...
  ModelOutput forward(const torch::Tensor& tokens,
    ...
    if (onerec_params != nullptr) {
      const std::string cache_key = build_encoder_cache_key(input_params);
      if (onerec_params->is_encoder_forward) {
        ...
        if (cached_encoder_output.defined()) {
          std::lock_guard<std::mutex> lock(encoder_output_cache_mutex_);
          if (encoder_output_cache_.size() > 64 &&
              encoder_output_cache_.count(cache_key) == 0) {
            encoder_output_cache_.clear();
          }
          encoder_output_cache_[cache_key] = cached_encoder_output;
        }
      ...
      if (onerec_params->has_encoder_output) {
        std::lock_guard<std::mutex> lock(encoder_output_cache_mutex_);
        const auto it = encoder_output_cache_.find(cache_key);
        if (it != encoder_output_cache_.end()) {
          cached_encoder_output = it->second;
        }
      }

      torch::Tensor decoder_context = onerec_params->decoder_context_embedding;
      if (!decoder_context.defined()) {
        decoder_context = cached_encoder_output;
      }

      if (!decoder_context.defined()) {
        LOG(ERROR) << "OneRec decoder requires encoder context.";
        return ModelOutput();
      }

      auto decoder_output =
          decoder_(tokens, positions, kv_caches, input_params, decoder_context);
 ...
 private:
#if defined(USE_NPU)
  static std::string build_encoder_cache_key(
      const ModelInputParams& input_params) {
    if (!input_params.request_ids.empty()) {
      std::string key;
      key.reserve(input_params.request_ids.size() * 24);
      for (const auto& request_id : input_params.request_ids) {
        key.append(request_id);
        key.push_back('|');
      }
      return key;
    }
    return "batch:" + std::to_string(input_params.batch_id);
  }
#endif
  ...
  std::unordered_map<std::string, torch::Tensor> encoder_output_cache_;
  std::mutex encoder_output_cache_mutex_;
#endif
};
