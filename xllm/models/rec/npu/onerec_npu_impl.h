...
inline torch::Tensor compute_onerec_position_bias(
    ...
    const ForwardInput* forward_input = nullptr) {
  ...
  if (is_decode_stage && forward_input != nullptr &&
      !forward_input->attention.host.kv_seq_lens.empty()) {
    const int32_t seq_kv_len = forward_input->attention.host.kv_seq_lens[0];
 ...
 public:
  ...
  torch::Tensor forward(const torch::Tensor& tokens,
    ...
    if (onerec_params->is_hybrid_mode && !is_decoder_) {
      ...
      if (h.device().type() == torch::DeviceType::PrivateUse1 &&
    ...
    auto [query_length, key_length] = compute_sequence_lengths(
        forward_input.meta.q_max_seq_len, is_prefill, forward_input);

    ForwardInput input_local = forward_input;
    ...
    for (size_t i = 0; i < layers_.size(); ++i) {
      ...
      if (forward_input.parallel.layer_synchronizer) {
      ...
      if (forward_input.parallel.layer_synchronizer != nullptr &&
          !forward_input.parallel.layer_synchronizer->synchronize_layer(i)) {
        return torch::Tensor();
      }
 ...
 private:
    ...
    if (is_decoder_) {
      if (is_prefill) {
        ...
        if (!forward_input.attention.host.kv_seq_lens.empty()) {
          key_length = *std::max_element(
              forward_input.attention.host.kv_seq_lens.begin(),
  ...
  torch::Tensor create_moe_attention_mask(int64_t seq_length,
    ...
    if (use_legacy_onerec_prefill_only_contract()) {
      ...
      return torch::triu(torch::ones({seq_length, seq_length},
    ...
    torch::Tensor upper_tri_mask =
        torch::triu(torch::ones({seq_length, seq_length},
                                torch::dtype(h.dtype()).device(h.device())),
                    /*diagonal=*/1);
    torch::Tensor expanded_mask = upper_tri_mask.unsqueeze(0).expand(
