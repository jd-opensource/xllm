  ...
  ModelOutput forward(const ForwardInput& input,
    ...
    if (onerec_params != nullptr) {
      if (onerec_params->is_encoder_forward) {
        ...
        auto encoder_output =
            encoder_(tokens, positions, encoder_kv_caches, input);
        ...
        if (encoder_output.defined() &&
          ...
          cached_encoder_output = pad_encoder_output(encoder_output, input);
        } else {
      ...
      auto decoder_output =
          decoder_(tokens, positions, kv_caches, input, cached_encoder_output);
