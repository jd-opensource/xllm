 ...
 public:
  ...
  void init_encoder_graph_manager(const ModelArgs& args,
    ...
    while (std::getline(iss, tok, ',')) {
      if (tok.empty()) continue;  // tolerate trailing/empty tokens
  ...
  // ---- VisionEncoderGraphAdapter ----
  // The manager captures/replays the encoder loop through this interface, so it
  // never depends on Qwen3-specific layer types.
