 ...
 public:
  ...
  void process(std::string& prompt, const MMData& mm_data) override {
    ...
    while (pair.second != std::string::npos) {
      if (pair.first == TokenType::IMAGE) {
        ...
        const int token_num = video_grid_thw[video_index][1].item<int>() *
                              video_grid_thw[video_index][2].item<int>() /
                              merge_length;
  ...
  void find_mm_spans(const std::vector<int>& prompt_tokens,
    ...
    while (true) {
      ...
      if (vision_start_it == prompt_tokens.end()) {
        break;
      }
      auto vision_end_it = std::find(
          vision_start_it + 1, prompt_tokens.end(), vision_end_token_id_);
      CHECK(vision_end_it != prompt_tokens.end());

      offset = std::distance(prompt_tokens.begin(), vision_start_it);
      length = std::distance(vision_start_it + 1, vision_end_it);

      int first_token = *(vision_start_it + 1);
      if (first_token == image_token_id_) {
        ...
        if (video_frames_left == 0) {
          ...
          video_frames_left = video_grid_thw[video_index][0].item<int>();
 ...
 private:
  ...
  std::vector<double> build_timestamps(const std::vector<double>& timestamps,
    ...
    for (size_t i = 0; i < ts.size(); i += static_cast<size_t>(merge_size)) {
      out.push_back((ts[i] + ts[i + static_cast<size_t>(merge_size) - 1]) /
                    2.0);
    }
