...
class Qwen3_5ModelImpl final
 ...
 public:
  ...
  virtual ModelOutput forward(const ForwardInput& forward_input,
    ...
    ForwardInput input_new = forward_input;
    std::vector<torch::Tensor> deep_stacks;

    if (dp_size_ > 1) {
      if (input_new.token_ids.numel() == 0) {
        input_new.token_ids = torch::tensor({1})
                                  .to(torch::kInt32)
                                  .to(forward_input.token_ids.device());
        input_new.positions = torch::tensor({1})
                                  .to(torch::kInt32)
                                  .to(forward_input.positions.device());
      }
