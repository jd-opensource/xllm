...
bool WhisperFeatureExtractor::process_audio(
  ...
  for (size_t feat_idx = 0; feat_idx < audio_feature_items.size(); feat_idx++) {
    ...
    if (!return_attention_mask_) {
      ...
      auto feat_origin_lens =
          torch::tensor({input_features.size(1)}, torch::kLong);
      auto feat_length = get_feat_extract_output_lengths(feat_origin_lens);
