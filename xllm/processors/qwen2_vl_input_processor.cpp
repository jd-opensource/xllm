...
torch::Tensor Qwen2VLInputProcessor::sample_frames(
  ...
  if (set_fps > 0.0 && num_frames > 0) {
    LOG(ERROR) << "num_frames and fps are mutually exclusive arguments, please "
                  "use only one!";
    return torch::Tensor();
  }
  ...
  if (num_frames > total_num_frames) {
    LOG(ERROR) << "Video can't be sampled. The inferred num_frames="
  ...
  if (num_frames > 0) {
    ...
    return torch::arange(
        0, total_num_frames, torch::TensorOptions().dtype(torch::kLong));
  }
}
