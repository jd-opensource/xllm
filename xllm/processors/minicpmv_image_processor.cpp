...
MiniCPMVImageProcessor::MiniCPMVImageProcessor(
  ...
  max_slice_nums_ = model_args->vision_max_slice_nums();
  scale_resolution_ = model_args->mm_scale_resolution();
  patch_size_ = model_args->mm_patch_size();
  slice_mode_ = model_args->mm_slice_mode();
  image_feature_size_ = model_args->mm_image_feature_size();
  norm_mean_ = model_args->mm_image_normalize_mean();
  norm_std_ = model_args->mm_image_normalize_std();
}
