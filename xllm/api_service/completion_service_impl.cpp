...
void CompletionServiceImpl::process_async_rpc_impl(
  ...
  ContinuousScheduler* scheduler = master_->get_continuous_scheduler();
  const auto& service_request_id = request->service_request_id();
  auto callback = [scheduler,
                   master = master_](const RequestOutput& req_output) -> bool {
    ...
    if (req_output.status.has_value()) {
      ...
      if (!status.ok()) {
        ...
        // finished with error.
        LOG(INFO) << "Completion request failed: " << status.message();
        master->get_rate_limiter()->decrease_one_request();
        return scheduler->send_stream_generation(req_output);
      }
    }
