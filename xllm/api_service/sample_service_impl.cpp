...
constexpr char kSelectorMatchFinishReason[] = "selector_match";
...
void vlog_sample_slots(const std::string& request_id,
                       const std::vector<SampleSlot>& sample_slots) {
  if (!VLOG_IS_ON(1)) {
    return;
  }
  for (const auto& slot : sample_slots) {
    VLOG(1) << "Sample request slot, request_id=" << request_id
            << ", sample_id=" << slot.sample_id
            << ", token_position=" << slot.token_position;
  }
}
...
bool build_request_params(const proto::SampleRequest& request,
  ...
  LOG(INFO) << "Sample request params built, request_id=" << params.request_id
            << ", selector_type=" << request.selector().type()
            << ", selector_value=" << request.selector().value()
            << ", sample_slots=" << params.sample_slots.size();
  vlog_sample_slots(params.request_id, params.sample_slots);
...
bool SampleServiceImpl::process_request(const proto::SampleRequest& request,
  ...
  *status = get_rate_limit_status(master_);
  ...
  log_dispatched_request(request_id,
                         request.model(),
                         match_count,
                         get_requested_logprobs(request));
