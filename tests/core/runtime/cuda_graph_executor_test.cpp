...
class ScopedConfigSnapshot final {
 public:
  ...
  ~ScopedConfigSnapshot() {
    ...
    KVCacheConfig::get_instance() = kv_cache_config_;
    ModelConfig::get_instance() = model_config_;
    SchedulerConfig::get_instance() = scheduler_config_;
  }
...
ForwardInput make_forward_input(const torch::Tensor& tokens,
                                const torch::Tensor& positions,
                                const ForwardInput& base_forward_input) {
  ForwardInput forward_input = base_forward_input;
...
class FakeAttnCausalLM final : public CausalLM {
 public:
  ...
  ModelOutput forward(const ForwardInput& forward_input,
    ...
    if (forward_input.attn_metadata) {
      attn_meta = *forward_input.attn_metadata;
    } else {
      attn_meta = layer::AttentionMetadataBuilder::build(forward_input,
                                                         /*enable_mla=*/false);
    }
...
ForwardInput make_decode_input(const torch::Device& device) {
  ForwardInput forward_input;
  forward_input.meta.batch_forward_type = BatchForwardType::DECODE;
  forward_input.meta.num_sequences = 1;
  forward_input.meta.kv_max_seq_len = 4;
  forward_input.meta.q_max_seq_len = 1;
  forward_input.enable_graph =
      false;  // executor will set metadata->enable_cuda_graph

  auto iopt = torch::TensorOptions().dtype(torch::kInt32).device(device);
  // cumulative lengths (cu_seq_lens)
  forward_input.attention.device.q_seq_lens = torch::tensor({0, 1}, iopt);
  forward_input.attention.device.kv_seq_lens = torch::tensor({0, 4}, iopt);
...
ForwardInput make_prefill_input(const torch::Device& device,
                                int32_t num_tokens) {
  ...
  forward_input.meta.num_sequences = 1;
  forward_input.meta.kv_max_seq_len = num_tokens;
  forward_input.meta.q_max_seq_len = num_tokens;
  forward_input.enable_graph =
      false;  // executor will set metadata->enable_cuda_graph
  ...
  // cumulative lengths (cu_seq_lens)
  forward_input.attention.device.q_seq_lens =
      torch::tensor({0, num_tokens}, iopt);
  ...
  forward_input.attention.device.new_cache_slots =
      torch::arange(0, num_tokens, iopt);
  forward_input.attention.device.block_tables =
...
ForwardInput make_multi_sequence_decode_input(const torch::Device& device) {
  ForwardInput forward_input;
  forward_input.meta.batch_forward_type = BatchForwardType::DECODE;
  forward_input.meta.num_sequences = 2;
  forward_input.meta.kv_max_seq_len = 9;
  forward_input.meta.q_max_seq_len = 1;
  forward_input.enable_graph = false;

  torch::TensorOptions iopt =
      torch::TensorOptions().dtype(torch::kInt32).device(device);
  forward_input.attention.device.q_seq_lens = torch::tensor({0, 1, 2}, iopt);
  forward_input.attention.device.kv_seq_lens = torch::tensor({0, 4, 9}, iopt);
  forward_input.attention.device.q_cu_seq_lens =
      forward_input.attention.device.q_seq_lens;
  forward_input.attention.device.new_cache_slots = torch::tensor({5, 7}, iopt);
  forward_input.attention.device.block_tables =
      torch::tensor({{0, 1, 2, 3}, {4, 5, 6, 7}}, iopt);
  forward_input.attention.device.paged_kv_indptr =
      torch::tensor({0, 1, 3}, iopt);
  forward_input.attention.device.paged_kv_indices =
      torch::tensor({2, 4, 6}, iopt);
  forward_input.attention.device.paged_kv_last_page_len =
      torch::tensor({1, 2}, iopt);
  return forward_input;
}
...
TEST(CudaGraphExecutorTest, DecodeMetadataFastPathUpdatesPersistentBuffers) {
  ...
  ForwardInput forward_input = make_multi_sequence_decode_input(device);
  forward_input.token_ids = tokens;
  forward_input.positions = positions;
  ...
  std::optional<ForwardInput> updated =
      persistent.update(forward_input,
                        kv[0].get_k_cache(),
                        kv[0].get_v_cache(),
                        /*padded_num_tokens=*/4,
  ...
  EXPECT_TRUE(torch::equal(updated->attn_metadata->block_table.cpu(),
                           forward_input.attention.device.block_tables.cpu()));
}
...
TEST(CudaGraphExecutorTest, DecodeMetadataFastPathUpdatesLinearStateIndices) {
  ...
  forward_input.token_ids = tokens;
  forward_input.positions = positions;
  forward_input.embedding.linear_state_ids = {8, 6};
  forward_input.embedding.linear_state_indices = torch::tensor({8, 6}, iopt);
  std::vector<KVCache> kv = MakeKvCaches(device,
                                         /*num_pages=*/16,
                                         /*page_size=*/1,
                                         /*num_kv_heads=*/1,
...
TEST(CudaGraphExecutorTest, DecodeMetadataFastPathFallbackMatchesLegacyPath) {
  ...
  torch::Tensor tokens = torch::tensor({10, 11}, iopt);
  torch::Tensor positions = torch::tensor({20, 21}, iopt);
  ForwardInput fast_input = make_multi_sequence_decode_input(device);
  ForwardInput fallback_input = make_multi_sequence_decode_input(device);
...
TEST(CudaGraphExecutorTest, BatchDecodeCaptureAndReplay) {
  ...
  ExecutionConfig::get_instance().enable_graph_vmm_pool(false);
  ...
  auto forward_input = make_decode_input(device);
  ...
  auto kv = MakeKvCaches(device,
                         ...
                         /*head_dim=*/128);
  auto eager_out =
      model->forward(make_forward_input(tokens, positions, forward_input), kv)
          .hidden_states.clone();
  torch::cuda::synchronize();
  // LOG(INFO) << "eager_out: " << eager_out;
  // Graph capture (first run) + replay (second run).
  auto out1 =
      graph_exec->run(make_forward_input(tokens, positions, forward_input), kv)
          .hidden_states;
  torch::cuda::synchronize();
  // LOG(INFO) << "out1: " << out1;
...
TEST(CudaGraphExecutorTest, PrefillPiecewiseCaptureAndReplay) {
  ...
  auto forward_input = make_prefill_input(device, kNumTokens);
  ...
  auto eager_out =
      model
          ->forward(make_forward_input(tokens, positions, forward_input),
                    kv_eager)
          .hidden_states.clone();
  torch::cuda::synchronize();

  auto out1 = graph_exec
                  ->run(make_forward_input(tokens, positions, forward_input),
                        kv_graph_first)
                  .hidden_states;
  out1 = out1.clone();
  torch::cuda::synchronize();

  auto out2 = graph_exec
                  ->run(make_forward_input(tokens, positions, forward_input),
                        kv_graph_second)
                  .hidden_states;
  out2 = out2.clone();
...
TEST(CudaGraphExecutorTest, CompareMqa2v1AndMqa8v1) {
  ...
  ExecutionConfig::get_instance()
      ...
      .max_tokens_for_graph_mode(128);
  SchedulerConfig::get_instance().max_tokens_per_batch(128);
  ModelConfig::get_instance().flashinfer_workspace_buffer_size(256 * 1024 *
                                                               1024);
...
TEST(CudaGraphExecutorTest, GraphVmmPoolMemoryReuseAcrossMultiShape) {
  ...
  ExecutionConfig::get_instance()
      .enable_graph(true)
      .enable_prefill_piecewise_graph(true)
      .max_tokens_for_graph_mode(256);
  SchedulerConfig::get_instance().max_tokens_per_batch(256);
  ...
  auto run_prefill_capture_sweep = [&](bool enable_graph_vmm_pool) {
    ExecutionConfig::get_instance().enable_graph_vmm_pool(
    ...
    for (int32_t num_tokens = 256; num_tokens >= 128; num_tokens -= 16) {
      ...
      auto forward_input = make_prefill_input(device, num_tokens);
      auto out =
          exec->run(make_forward_input(tokens, positions, forward_input), kv)
              .hidden_states;
      (void)out;
      torch::cuda::synchronize();
  ...
  for (int32_t num_tokens = 128; num_tokens <= 256; num_tokens += 16) {
    ...
    auto eager_out =
        model
            ...
            .hidden_states.clone();
    auto graph_out =
        exec->run(make_forward_input(tokens, positions, forward_input),
                  kv_graph)
            .hidden_states.clone();
    torch::cuda::synchronize();
