 ...
 public:
  ...
  torch::Tensor forward_impl(const torch::Tensor& tokens,
    ...
    LOG(INFO) << "SimpleCausalLM forward_impl, tokens: " << tokens.sizes()
              ...
              << ", params: " << forward_input.meta.num_sequences;
    ...
    if (forward_input.attention.device.new_cache_slots.defined()) {
      ...
      auto cache_slots_sum =
          torch::sum(forward_input.attention.device.new_cache_slots);
    ...
    if (forward_input.attention.device.block_tables.defined() &&
      ...
      auto max_seq_len = torch::max(forward_input.attention.device.kv_seq_lens);
      ...
      auto kv_embeddings = torch::embedding(
          kv_cache_tensor, forward_input.attention.device.block_tables);
  ...
  ModelOutput forward(const ForwardInput& forward_input,
    ...
    auto hidden_states = forward_impl(forward_input.token_ids,
