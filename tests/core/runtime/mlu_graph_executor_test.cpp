...
class MockCausalLM : public CausalLM {
 public:
  ...
  ModelOutput forward(const ForwardInput& input,
    ...
    last_dp_token_nums_ = input.parallel.dp_global_token_nums;
    auto hidden_states = input.embedding.input_embedding.matmul(weight_);
...
class MluGraphExecutorTest : public ::testing::Test {
 protected:
  ...
  ForwardInput prepare_inputs(int32_t batch_size, uint64_t seed) {
    ...
    input.meta.kv_max_seq_len = 1;
    input.meta.q_max_seq_len = 1;
    input.parallel.dp_global_token_nums = {1};
    input.parallel.dp_is_decode = {1};
    input.attention.device.new_cache_slots = new_cache_slots;
    input.attention.device.block_tables = block_table;
    input.attention.device.q_seq_lens = q_seq_lens;
    input.attention.device.kv_seq_lens = kv_seq_lens;
    input.attention.host.q_seq_lens = q_seq_lens_vec;
    input.attention.host.kv_seq_lens = kv_seq_lens_vec;
    input.embedding.input_embedding = input_embedding;
