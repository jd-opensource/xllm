...
class DeepseekV2DecoderLayerTest : public ::testing::Test {
 protected:
  ...
  ProcessGroup* build_single_rank_pg() {
    single_rank_pg_ = std::make_unique<test::MockProcessGroup>(
        options_.device(), /*rank=*/0, /*world_size=*/1);
    return single_rank_pg_.get();
  }
  ...
  ForwardInput build_prefill_params(int64_t batch_size, int64_t seq_len) {
    ForwardInput input;
    input.meta.batch_forward_type = BatchForwardType::PREFILL;
    input.meta.num_sequences = batch_size;
    input.meta.q_max_seq_len = seq_len;
    input.meta.kv_max_seq_len = batch_size * seq_len;
    input.attention.device.q_seq_lens = torch::arange(
        0,
        (batch_size + 1) * seq_len,
        seq_len,
        torch::TensorOptions().dtype(torch::kInt32).device(options_.device()));
    input.attention.device.kv_seq_lens = torch::arange(
        0,
        (batch_size + 1) * seq_len,
        seq_len,
        torch::TensorOptions().dtype(torch::kInt32).device(options_.device()));
    input.attention.host.q_seq_lens.resize(batch_size, seq_len);
    input.attention.host.kv_seq_lens.resize(batch_size, seq_len);
    input.attention.device.new_cache_slots = torch::arange(
        1,
        batch_size * seq_len + 1,
        torch::TensorOptions().dtype(torch::kInt32).device(options_.device()));
    input.attention.device.block_tables = torch::zeros(
        {batch_size, batch_size * seq_len},
        torch::TensorOptions().dtype(torch::kInt32).device(options_.device()));

    for (int64_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
      const int64_t start = batch_idx * seq_len + 1;
      input.attention.device.block_tables[batch_idx].index_put_(
    ...
    return input.to(options_.device());
  }
...
TEST_P(DeepseekV2DecoderCarrierTest,
  ...
  ForwardInput input;
  input.parallel.dp_global_token_nums = tc.dp_global_token_nums;
...
TEST_F(DeepseekV2DecoderLayerTest, RestoreFfnOutputPackedLocal) {
  ...
  auto restored = DeepseekV2DecoderLayerTestPeer::restore_ffn_output(
      *decoder, packed_ffn_out, carrier, input);
  test::verify_tensor_close(
      restored,
...
TEST_F(DeepseekV2DecoderLayerTest, ForwardMixedDpMoEReturnsLocalSlice) {
  ...
  ForwardInput input;
  input.meta.batch_forward_type = BatchForwardType::PREFILL;
  input.meta.num_sequences = 2;
  input.meta.q_max_seq_len = 1;
  input.meta.kv_max_seq_len = 2;
  input.attention.device.q_seq_lens = torch::tensor(
      {0, 1, 2},
      torch::TensorOptions().dtype(torch::kInt32).device(options_.device()));
  input.attention.device.kv_seq_lens = torch::tensor(
      {0, 1, 2},
      torch::TensorOptions().dtype(torch::kInt32).device(options_.device()));
  input.attention.host.q_seq_lens = {1, 1};
  input.attention.host.kv_seq_lens = {1, 1};
  input.attention.device.new_cache_slots = torch::arange(
      1,
      3,
      torch::TensorOptions().dtype(torch::kInt32).device(options_.device()));
  input.attention.device.block_tables = torch::tensor(
      {{1, 0}, {2, 0}},
      torch::TensorOptions().dtype(torch::kInt32).device(options_.device()));
  input.parallel.dp_global_token_nums = {2, 1};
  input.parallel.dp_is_decode = {0, 0};
  input = input.to(options_.device());

  auto attn_metadata = AttentionMetadataBuilder::build(input.meta,
                                                       input.attention,
                                                       input.graph,
                                                       input.llmrec_params(),
...
TEST_F(DeepseekV2DecoderLayerTest, DenseMlpChunkMatchesDirectPrefill) {
  ...
  auto hidden_states = test::seeded_tensor("deepseek_v2_decoder.chunk_prefill",
                                           ...
                                           torch::kBFloat16,
                                           options_.device());
  auto input = build_prefill_params(/*batch_size=*/1, /*seq_len=*/5);

  auto expected =
      DeepseekV2DecoderLayerTestPeer::mlp(*decoder)->forward(hidden_states);
  auto actual = DeepseekV2DecoderLayerTestPeer::run_mlp(
      *decoder, hidden_states, input.meta.batch_forward_type);
...
TEST_P(DeepseekV2DecoderLayerParamTest,
  ...
  auto positions = torch::arange(
      ...
      kBatchSize * kSeqLen,
      torch::TensorOptions().dtype(torch::kInt32).device(options_.device()));
  auto input = build_prefill_params(kBatchSize, kSeqLen);
  auto attn_metadata = AttentionMetadataBuilder::build(input.meta,
                                                       input.attention,
                                                       input.graph,
                                                       input.llmrec_params(),
