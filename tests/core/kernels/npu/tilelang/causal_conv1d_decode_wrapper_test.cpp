...
class TileLangCausalConv1dDecodeTest : public ::testing::Test {
 protected:
...
torch::Tensor causal_conv1d_decode_cpu_ref(const torch::Tensor& x,
  ...
  auto x_f = x.to(torch::kFloat32).cpu();
  auto weight_f = weight.to(torch::kFloat32).cpu();
  auto conv_state_f = conv_state.to(torch::kFloat32).cpu().clone();
  auto bias_f = bias.to(torch::kFloat32).cpu();
  auto init_l = init_indices.to(torch::kInt64).cpu();
  auto current_l = current_indices.to(torch::kInt64).cpu();
...
void run_causal_conv1d_decode_case(
  ...
  auto x = torch::randn({test_case.batch_size, test_case.dim}, bf16_opts);
  auto conv_state = torch::randn(
      {test_case.num_cache_lines, test_case.dim, kStateLen}, bf16_opts);
  auto weight = torch::randn({test_case.dim, kWidth}, bf16_opts);
  auto bias = torch::zeros({test_case.dim}, bf16_opts);
  ...
  auto x_cpu = x.cpu();
  auto cs_cpu = conv_state.cpu().clone();
  auto w_cpu = weight.cpu();
  auto b_cpu = bias.cpu();
  ...
  auto weight_t = prepare_weight_t(weight);
  auto conv_state_t = prepare_conv_state_t(conv_state);
  ...
  auto max_diff = (y.cpu().to(torch::kFloat32) - golden.to(torch::kFloat32))
                      ...
                      .max()
                      .item<float>();
  std::cout << "[causal_conv1d_decode_test] case=" << test_case.name
...
TEST_F(TileLangCausalConv1dDecodeTest, DecodeBatch1Dim2048) {
  run_causal_conv1d_decode_case({
      .name = "decode_bs1_d2048",
      .batch_size = 1,
      .dim = 2048,
      .has_silu = true,
      .num_cache_lines = 4,
...
TEST_F(TileLangCausalConv1dDecodeTest, DecodeBatch1Dim4096) {
  run_causal_conv1d_decode_case({
      .name = "decode_bs1_d4096",
      .batch_size = 1,
      .dim = 4096,
      .has_silu = true,
      .num_cache_lines = 4,
...
TEST_F(TileLangCausalConv1dDecodeTest, DecodeBatch1Dim8192) {
  run_causal_conv1d_decode_case({
      .name = "decode_bs1_d8192",
      .batch_size = 1,
      .dim = 8192,
      .has_silu = true,
      .num_cache_lines = 4,
...
TEST_F(TileLangCausalConv1dDecodeTest, DecodeBatch4Dim2048) {
  run_causal_conv1d_decode_case({
      .name = "decode_bs4_d2048",
      .batch_size = 4,
      .dim = 2048,
      .has_silu = true,
      .num_cache_lines = 8,
...
TEST_F(TileLangCausalConv1dDecodeTest, DecodeBatch4Dim4096) {
  run_causal_conv1d_decode_case({
      .name = "decode_bs4_d4096",
      .batch_size = 4,
      .dim = 4096,
      .has_silu = true,
      .num_cache_lines = 8,
