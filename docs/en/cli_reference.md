...
| `enable_prefill_sp` | `bool` | false | true | Whether to enable DeepSeek V3.2 prefill-only sequence parallel. | Supported only for `deepseek_v32` in the current branch; for now, only support `enable_prefix_cache=false` plus `enable_chunked_prefill=false`. |
