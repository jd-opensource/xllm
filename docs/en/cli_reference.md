...
| `enable_prefill_sp` | `bool` | false | true | Whether to enable DeepSeek V3.2 prefill-only sequence parallel. | Supported only for `deepseek_v32` in the current branch. Requires `enable_prefix_cache=false`. `enable_chunked_prefill=true` is supported only for prefill-only batches (`PREFILL` / `CHUNKED_PREFILL`); `MIXED` and `DECODE` batches do not run with sequence parallel. |
