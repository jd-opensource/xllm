...
## LM Head 候选裁剪与 Qwen3 Reranker 参数
| 参数名称 | 类型 | 默认值 | 其他值 | 参数含义 | 其他 |
|:---------:|:---------:|:---------:|:---------:|:---------:|:---------:|
| `candidate_token_ids` | `string` | "" | 例如 `2152,9693` | 候选 token id 列表（逗号分隔）。启用后 `lm_head` 会按候选集合裁剪输出维度。 | 推荐线上优先使用 |
| `enable_qwen3_reranker` | `bool` | false | true | 是否启用 qwen3 reranker 服务语义。 | 开启时要求候选集去重后 `K==2` |
