...
| `enable_prefill_sp` | `bool` | false | true | 是否开启 DeepSeek V3.2 仅 prefill 阶段的 sequence parallel | 仅当前分支的 `deepseek_v32` 生效；要求 `enable_prefix_cache=false`。支持 `enable_chunked_prefill=true`，但仅限纯 prefill batch（`PREFILL` / `CHUNKED_PREFILL`）；`MIXED` 与 `DECODE` batch 不会进入 sequence parallel。 |
