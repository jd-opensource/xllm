...
def _install_dependencies(
    ...
    command = f"sh third_party/dependencies.sh {dependency} {device}"
...
def pre_build(device: Optional[str] = None) -> None:
    script_path = os.path.dirname(os.path.abspath(__file__))
    normalized_device = (device or "").lower()

    _validate_submodules_or_exit(script_path)
    _install_dependencies(script_path, "yalantinglibs", normalized_device)
