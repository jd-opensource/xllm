...
def _get_ascend_opp_root() -> str:
    if ascend_home := os.getenv("ASCEND_HOME_PATH"):
        return os.path.join(
            os.path.abspath(os.path.expanduser(ascend_home)),
            "opp",
        )
    if ascend_opp_path := os.getenv("ASCEND_OPP_PATH"):
        return os.path.abspath(os.path.expanduser(ascend_opp_path))

    return "/usr/local/Ascend/ascend-toolkit/latest/opp"
...
def _get_cann_version_info() -> tuple[str, Optional[str]]:
    version_info_path = os.path.join(
        os.path.dirname(_get_ascend_opp_root()),
        "compiler",
        "version.info",
    )
